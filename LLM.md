...
- [ ] **Multi-tenant SaaS foundation — WAVE 1 (2026-09-04, see Build Log)** — plan: `~/.claude/plans/task-multi-tenant-saas-foundation-purring-parnas.md`
...
- [x] **Multi-tenant SaaS foundation — WAVE 3, platform plane (2026-09-04, see Build Log)** — plan §2/§3, same plan doc as wave 1. Built concurrently with the wave-2a worktree above; stayed strictly inside new `platform`-namespace files plus read-only `db` imports, per the task brief's file-collision guard.
  ...
  - [x] TOTP (RFC 6238) via `otpauth` (added to `package.json`, chosen for zero native deps + edge/node portability): `src/lib/platform-totp.ts`. A `PlatformUser` with `totpSecret`/`totpConfirmedAt` unset is HARD-BLOCKED at login, not "let in and prompt setup" — enforced in `platform-auth.ts`'s `authorize()`. First-time setup is out-of-band via `scripts/create-platform-user.mjs create`/`confirm`, mirroring `create-admin-user.mjs`'s existing bootstrap convention — deliberately no unauthenticated web setup route.
