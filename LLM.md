...
- [ ] **Multi-tenant SaaS foundation — WAVE 1 (2026-09-04, see Build Log)** — plan: `~/.claude/plans/task-multi-tenant-saas-foundation-purring-parnas.md`
  ...
  - [ ] Migration never run against a real DB from this environment (no Postgres available here, same standing limitation as every other hand-authored migration in this repo) — verify with `prisma migrate deploy` naming the migration explicitly before trusting it.
  - [ ] Rehearsal itself never run (needs a restored snapshot + real Postgres) — script is written and ready, not yet exercised.
