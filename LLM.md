...
- Premium "Hybrid AI + Human" plan (§34, 2026-09-14): the AI voice engine connects to Asterisk via **AudioSocket**, never ARI — the AMI-not-ARI decision (§6) stays locked. An AI-kind `Extension` never gets a `User`/login/password. AI agents are **not** geo-locked (no login location to check); what's locked instead is admin access to AI settings/keys, per-tenant provider region, and outbound destination/hours. `AiAgent.outboundEnabled` ships `false` — see §34 for the India TRAI / UAE TDRA reasoning. **Known architecture limit, PARTIALLY addressed (§34.1, 2026-09-14):** `AiAgent.dinstarPort` (1-4, matching `WaInstance.simPort`'s numbering) plus `@@unique([tenantId, dinstarPort])` now lets each tenant run up to 4 AI agents, one per Dinstar GSM port, instead of one globally — the cross-tenant "only one tenant enabled at a time" guard in `POST/PATCH /api/admin/ai/agents` is UNCHANGED (it only ever blocked cross-tenant, never same-tenant, so it didn't need to). What's still not fixed: the dialplan's `DINSTAR_PORT` channel variable that would make the per-port routing live has no real value to read from today — the gateway still presents all 4 ports through one shared Port Group/trunk with no live-confirmed per-port signal (§25.1). See §34.1 for exactly what's built vs. still a TODO.
...
- [x] **Multi-tenant SaaS foundation — WAVES 4-7 + the owner console (2026-09-06, see Build Log)** — plan §4/§5/§6/§7, same plan doc as waves 1-3. Six-section console at `/platform`, the billing ladder, provisioning, and the per-tenant recording delivery pipeline (an owner-expanded scope decision, see below).
  - [x] **(2026-09-08, §33)** Owner tenant-management modal (row click → subscription/users/geo/extensions), provisioning overview-before-edit-wizard, per-user VPN profiles (`/admin/users/[id]`, tenant admin only), and CRM completion (deal↔company picker, task creation, threaded notes on deal/company/contact wired into the unified timeline, CRM analytics surfaced on `/admin` itself). Gated green (typecheck/910 tests/lint/build) and **DEPLOYED to production the same day** — see §33 for the full breakdown.
...
- 2026-09-05 — **Ran the platform (owner console) Playwright acceptance suite for the first time ever.** It had been written, typechecked and shelved because the build machine had no database and no Docker; Docker is available now, so it was run against a local Postgres 16 (all migrations applied clean) with a seeded, TOTP-enrolled `PLATFORM_OWNER`. First run: 20 passed / 4 failed. All four are fixed (`33b710c`), final run **28 passed, 0 failed, 5 skipped** — the skips are the tenant-role specs, which skip by design without `E2E_ADMIN_*`.
  ...
  - [x] **DEPLOYED TO PRODUCTION 2026-09-07.** Owner go-ahead given; the VPS working tree moved `a37f2e7` → `4e8fc91` (22 commits), rebuilt with `docker compose build --no-cache web` (one service, per the 2-vCPU constraint; `--no-cache` because this pull adds many new files and the cached-build-omits-new-files failure mode is recorded in memory), then `docker compose up -d web`. Three things the earlier pre-flight had NOT caught, each found and handled before the reset rather than after:
    - **`website/out` is a host-built artifact that is untracked AND absent from `origin/main`**, yet `docker-compose.yml` bind-mounts it to `/srv/website` and the live Caddyfile `root`s the apex at it. Deleting the untracked `website/` tree to clear the checkout conflict would have taken the working public site down. It was moved aside and restored after the reset; apex verified 200 before and after.
    - **`20260904140000_add_tenancy_constrain` is applied in the production DB but does not exist in `origin/main`.** It was deliberately KEPT as an untracked directory, so the migrations folder stays a superset of what the DB has; the container's start-up `prisma migrate deploy` then reported "29 migrations found / No pending migrations to apply". Had it been cleaned, deploy would have failed on an applied-but-missing migration.
    - The other untracked paths (`website/` sources, `app/platform/setup`, `app/api/platform/setup`, the `20260905090000` migration) DO exist upstream, so they were displaced rather than deleted — everything discarded is preserved under `/root/pre-console-deploy-20260907/` (full `git diff`, `git status`, an untracked tarball, and the displaced trees).
    - **Post-deploy verification:** `/api/health` `{"ok":true}`, `/login` 200, `/platform/login` 200, `/platform` 307 → `/platform/login?callbackUrl=%2Fplatform` (guard working), apex `saharatechs.com` 200 and `/terms` 200, `algo-web` healthy, zero errors in its logs, all 13 containers up.
...
- **§34's AI voice engine is not wired end-to-end yet.** Before selling the Hybrid AI plan to a real tenant: (1) ~~wire `ai-voice-agent/pipeline/registry.py` into `main.py`'s AudioSocket connection handler~~ — **DONE 2026-09-14**: added `ai-voice-agent/pipeline/runner.py`'s `PipelineRunner` (drives `build_realtime` for `REALTIME` mode, or a silence-endpointed STT→LLM→TTS cascade loop for `CASCADE` mode, greeting spoken via TTS at cascade start) and wired it into `main.py`'s `AudioSocketServer._handle_connection` (`runner.push_audio()` fed from every inbound SLIN frame, `runner.stop()` on hangup/close). 4 new unit tests (`tests/test_pipeline_runner.py`), 49/49 passing. **Known limitation, not fixed**: no mid-turn barge-in cancellation for the cascade path — inbound audio arriving while an LLM/TTS call is in flight just queues and gets included in the next utterance's STT input, rather than interrupting that turn; the existing `BargeInDetector` still flushes already-*enqueued* outbound audio, which covers the common case. This has never been exercised against a live AudioSocket connection or a real STT/LLM/TTS vendor — unit-tested with fakes only. (2) place one real inbound test call against a live Asterisk instance to verify the new `CURL()`/`AudioSocket()` dialplan syntax (unverified against a live instance, same caveat this file already carries for `DNC_CHECK`); (3) **§34.1 (2026-09-14) built the schema/API/UI/dialplan for per-Dinstar-port AI agent routing** (`AiAgent.dinstarPort` 1-4, `AI_INBOUND_EXTENSION(${DINSTAR_PORT})`) so a tenant can in principle run up to 4 AI agents at once — but it is NOT live-verified: `extensions.conf`'s `[from-dinstar]` sets `DINSTAR_PORT` to empty today because the Dinstar gateway still routes all 4 physical ports through one shared Port Group/trunk with no per-port signal visible to the dialplan (§25.1). Making it real needs either reconfiguring the gateway's `PortCfg`/`SipAccN` so each port registers as its own distinct SIP identity, or a live trace of what the gateway sends per port on the current shared-trunk config. Until then this still behaves like the old single-agent-at-a-time model in practice, even though the cross-tenant guard/schema no longer artificially cap it at one tenant's agent.
- **§34's `checkOutbound()` compliance gate (TRAI/UAE-TDRA) is unit-tested but not called from anywhere** — nothing originates an AI outbound call yet. Whoever builds that path must call it at the exact point of origination, non-bypassably, or the compliance design is decorative. Needs explicit sign-off before `AiAgent.outboundEnabled` is ever allowed to flip true for a real tenant.
...
- 2026-09-07 — **The owner console is LIVE on production.** Owner chose deploy over holding. VPS `/opt/algo-pbx` reset `a37f2e7` → `4e8fc91` (22 commits), `docker compose build --no-cache web` (exit 0), `docker compose up -d web`. Everything discarded is recoverable under `/root/pre-console-deploy-20260907/`. The pre-flight recorded in the previous session was accurate as far as it went, but three things it had not examined would each have caused real damage: (1) `website/out` — the static export that IS the live public site — is untracked and not in `origin/main`, so clearing the checkout conflict by deleting `website/` would have emptied `/srv/website`; it was moved aside and restored, apex verified 200 either side. (2) `20260904140000_add_tenancy_constrain` is applied in the production DB but absent from `origin/main`; keeping it as an untracked directory is what let start-up `prisma migrate deploy` report "29 migrations found / No pending migrations" instead of failing on an applied-but-missing migration. (3) `rm -rf` was declined by the harness for the conflicting paths, which turned out to be the better shape anyway — everything was `mv`'d into the backup directory instead, so the whole reset is reversible. Verified after: `/api/health` `{"ok":true}`, `/login` 200, `/platform/login` 200, `/platform` 307 → login with callback (guard live), apex 200, apex `/terms` 200, `algo-web` "Up (healthy)" with zero errors logged, all 13 containers up. **What this does NOT prove:** no real call has been placed since the deploy, and Gate 1b (a call completing while a tenant is suspended) remains the untested, load-bearing promise of the billing design.

## 33. Owner tenant management modal, provisioning overview, per-user VPN profiles, and CRM completion (deals/tasks/notes/dashboard) — built, gated green, and DEPLOYED to production (2026-09-08)

Plan: `~/.claude/plans/objective-the-owner-page-enchanted-sphinx.md`, designed as a task graph (G0 shared migration → parallel W1/W2/W6 → W3 → W4 → W5). Explored via 3 parallel `Explore` agents + 1 `Plan` agent before writing any code. Four user-reported gaps addressed:

**G0 — schema.** Additive migration `20260908120000_add_user_vpn_and_crm_notes`: new `UserVpnProfile` (1:1 per `User`, reuses `SiteTransport`/`SiteConnectivityStatus` enums, `configEncrypted` via the existing `src/lib/settings/crypto.ts` scheme — same as `GatewaySite.configEncrypted`, not a new one), new `CompanyNote` (exact structural mirror of the existing `DealNote`), `ContactTask.description String?`. Both new tables added to `TENANT_SCOPED_MODELS`/`TENANCY_TABLES`, deliberately not added to the RLS policy set (same considered-scope reasoning as the geo-lock migration). `npx prisma generate` clean.

**W1 — owner tenant management ("the click is not popping any window").** Discovered mid-build that `/platform/tenants/[id]`'s tab component (`TenantDetailTabs`) already covered subscription/users/geo management in full, including per-extension country allocation via a real `<Select>` dropdown (`geo-tab.tsx`) — the actual gap was narrower than first scoped: (a) tenant list rows were inert (only the slug cell linked anywhere), and (b) there was no Extensions tab (which user holds which extension, dial permission). Fixed both without duplicating the existing tabs: new `GET /api/platform/tenants/[id]/detail` re-serves `loadTenantDetail()` as JSON, a new client `TenantManageModal` fetches it and renders the SAME `<TenantDetailTabs/>` inside a `Dialog`; `tenant-row-actions.tsx` makes each row clickable (`data-testid`/`data-slug` preserved for existing e2e specs) while the slug `Link` still navigates to the full page. New `ExtensionsTab` (owner-only) + `PATCH .../extensions/[extensionId]` (`assign_user`/`unassign_user`/`set_dial_permission`, each reason-gated and audited) + pure `src/lib/platform/extension-assignment.ts` (`canAssignExtension`/`canUnassignExtension`, rejects assigning a user who already holds a different extension with a 409, not an opaque unique-constraint 500) — 9 new vitest cases. 3 new blast-radius strings + assertions (`blast-radius.test.ts`, copy-and-test-move-together per that file's own rule).

**W2 — provisioning ("only opening the detail edit page").** `/platform/provisioning/[id]` now renders a read-only `ProvisioningOverview` (progress bar, step list, prereq/blocker verdict from the page's own already-computed `canAdvance()` inputs) by default; the existing `ProvisioningWizard` only renders on `?mode=edit`, reached via an explicit "Open edit wizard" link, with "Back to overview" the other way. No new models/routes — pure UI-gating on a search param, all data loading unchanged.

**W3 — per-user VPN ("each user should have their own Tailscale/Headscale/OpenVPN/WireGuard setup page, only in the user admin page").** New `/admin/users/[id]` + `UserVpnPanel`, gated by `requireAdminSession()` — the guard, not the nav, is what keeps this off `/agent/**` (nothing under `src/app/agent/**` was touched). `POST/GET/DELETE /api/admin/users/[id]/vpn` + `GET .../vpn/config`: generates REAL X25519 keypairs (`src/lib/connectivity/wireguard-keypair.ts`, via Node's `generateKeyPairSync("x25519", ...)`, stripping the fixed SPKI/PKCS8 DER envelope to the raw 32-byte scalar `wg genkey` would produce — verified against Node's own output, not assumed), self-validates the generated config through the EXISTING `transports.WIREGUARD.validateConfig()` (`src/lib/connectivity/user-vpn-config.ts`) so it can never emit something the app's own validator would reject, and is honest that TAILSCALE/OPENVPN have no generatable config (`supportsGeneratedConfig()` returns false, matching this repo's existing "never fake a check you can't perform" convention from `wireguard.ts`'s own `probe()`). Tunnel IP allocation is a new generic /24 scanner (`src/lib/connectivity/user-vpn-ip.ts`, deliberately NOT reusing `src/lib/platform/subnet.ts`'s tenant-gateway-specific .1/.10 convention beyond borrowing its CIDR) starting at .20. Config is one-time-disclosed on POST, never re-served by GET (same discipline as `Extension.sipSecret`/`ApiKey`); download is a separate audited route. 15 new vitest cases across the two new lib files.

**W4 — CRM creation gaps.** Extracted `pipeline-board.tsx`'s inline `ContactPicker` into `src/components/crm/contact-picker.tsx` (also exporting a new `CompanyPicker` against the already-existing `GET /api/admin/crm/companies?q=`) so both the deal dialogs and the new task dialog share one search-picker implementation. Deal create/edit dialogs gained Company/Stage/Currency/Owner/Expected-close fields — all already accepted by `DealCreateSchema`/`DealPatchSchema`, so `src/lib/crm/deals.ts` needed zero changes. `task-board.tsx` gained a "New task" button + dialog (admin-plane only — the agent plane already creates tasks from the contact drawer via a different route) with a REQUIRED contact picker per the confirmed decision; new `POST /api/admin/crm/tasks` (route previously had only GET+PATCH) + `createTask()` in `tasks-data.ts` + pure `src/lib/crm/task-input.ts` (`normalizeTaskInput()`, rejects a due date >5 years out as a likely typo) — 9 new vitest cases.

**W5 — threaded notes ("no notes/description section in any of the crm pages").** Activated the `DealNote` model (declared in the schema since wave 1, zero code had ever used it) and added `CompanyNote`. New pure `src/lib/crm/notes.ts` (`toNoteThread()`, `noteActivitySummary()` reusing the existing `truncateBody()` so a deal/company note's timeline entry reads identically to a contact note's, `noteActivityRefId()` — deliberately just the note's own cuid, matching the existing agent contact-notes route's convention) — 10 new vitest cases. Three new GET/POST routes (`api/admin/crm/deals/[id]/notes`, `api/admin/crm/companies/[id]/notes`, `api/admin/contacts/[id]/notes`), each writing a matching `Activity` row via the existing `recordActivity()`. Honest gap documented in the company-notes route's own header: `Activity` has no `companyId` column — a company note's Activity row attaches to the company's most-recently-updated contact when one exists, and is skipped (only the note itself is written) when the company has none; no schema column was invented to route around this. New `NoteThread` component (composer + list, author + timestamp) reused verbatim on the deal edit dialog, the company detail panel, and the admin contacts edit-form panel.

**W6 — CRM dashboard ("bring the crm related dashboard ... as an analytical view with graphs").** Mid-build discovery: `/admin/reports`'s "CRM Insights" tab (`crm-tab.tsx`) already existed, fully built and wired to real API routes — pipeline funnel, dispositions, agent leaderboard, tasks due, top contacts, DNC trend, all on `recharts` (already a dependency). Rather than duplicate that chart logic into a new `dashboard-metrics.ts` (the plan's original shape), reused `<CrmTab/>` directly: new thin `AdminDashboardCrmPanel` (fixed 30-day default range via the existing `defaultFilterState()`, a "Full reports →" link for anyone who wants the interactive range/agent filters) rendered on `/admin` below the Wallboard in both its existing branches. Zero new API routes, zero new chart code.

**Gates before deploy, all green:** `npx prisma generate` clean; `npm run typecheck` clean; `npm run test` — 910/910 passing (43 new tests across 7 new pure-logic files, all colocated under `src/lib/`, literal fixtures, no DB, matching this repo's established test convention); `npm run lint` clean; `npm run build` — clean production build, all new routes present in the manifest (`/platform/tenants` 199 kB, `/platform/provisioning/[id]` 108 kB, `/platform/tenants/[id]` 197 kB, plus every new `/api/**` route). **Not run:** `prisma migrate diff` against a shadow DB (no reachable Postgres in the build environment — `DATABASE_URL` is a dummy placeholder), so the new migration's shape was syntax-reviewed and modeled on the existing geo-lock migration rather than diff-verified before deploy.

## 34. Premium "Hybrid AI + Human" plan — built as a task graph, 8 parallel subagents + a security/correctness fix round — NOT deployed, one architecture limit intentionally not fixed (2026-09-14)

User asked to fork one of `ictinnovations/asterisk-ai-voice-agent` (MIT) or `dograh-hq/dograh` (BSD-2) into a new AED 800/mo plan where a tenant admin allocates each of the tenant's ports (default lowered 5→4) to a human agent or an admin-managed, no-login AI voice agent, GSM-gateway-only (no Telnyx/cloud numbers), with API keys auto-fetching provider model lists and a considered answer on whether AI agents need geo-locking. Explored via 3 parallel `Explore`/research agents (codebase audit; Dograh-vs-ictinnovations comparison; low-latency-provider + India-TRAI/UAE-TDRA research) before planning. **Decision: forked `asterisk-ai-voice-agent`, not Dograh** — Dograh needs ARI (Stasis app, ExternalMedia), which would have reopened the locked AMI-not-ARI decision (§6); ictinnovations' AudioSocket-based design keeps it locked, at the cost of building out multi-provider/multi-tenant support ourselves (it shipped with only Claude/Whisper/Piper).

Plan written as a task graph (`~/.claude/plans/we-need-to-bring-pure-muffin.md`): N0 (schema/contracts, sequential root) → 5 parallel workers needing only N0 (W1 seats/plan, W3 provider registry, W4 Python sidecar fork, W5 Asterisk/compose, W6 internal APIs) → 2 more needing those (W7 admin UI, W8 compliance) → 3 independent verifiers in fresh contexts (V1 correctness, V2 security, V3 build/tests) → one fix round → this merge. Shared state lived in `.agents/hybrid-ai/{contracts.md,status.md}` in the repo root, deleted after merge.

**N0 — schema (`prisma/schema.prisma`, migration `20260914090000_add_hybrid_ai_agents`).** `Tenant.seats` default 5→4 (existing rows untouched — `ALTER COLUMN...SET DEFAULT` only affects future inserts). `Extension.agentType` (`"HUMAN"|"AI"` string, matching the existing `kind` field's plain-string convention) — an AI extension has `userId=null`, no `sipSecret`/`voicemailPin`. Three new tables: `AiAgent` (1:1 with an AI-kind `Extension`, prompt/greeting/language/pipelineMode + one leg per provider type + the compliance fields below), `AiProviderCredential` (per tenant+provider+label, key encrypted with the EXISTING `src/lib/settings/crypto.ts` AES-256-GCM scheme, not a new one — same reuse discipline as every prior session's crypto decisions), `AiCallSession` (linked to `CallDetailRecord` by `uniqueId`/`cdrUniqueId` string match, not an FK, since the CDR listener and the sidecar's session-report are two independent writers racing on the same call). All three added to `TENANT_SCOPED_MODELS`/`TENANCY_TABLES` (the two lists that MUST stay in sync per `scope-rules.ts`'s own header). Frozen contracts (internal API shapes, provider adapter interface, AudioSocket port numbers 9092/9091) written to `src/lib/ai/types.ts` + `.agents/hybrid-ai/contracts.md` before any worker started.

**W1 — seats/plan (`plan-catalog.ts`, `seat-guard.ts`, `admin/users` + `extensions` POST routes).** New `premium` plan entry (AED 800, `features.aiAgents`) + `planHasFeature()`. `assertSeatAvailable()`/`getSeatUsage()` count `Extension` rows regardless of `agentType`, throw `SeatLimitError`→409 (same throw-typed-Error-map-to-status convention as `audit.ts`'s `MissingReasonError`). **Known, accepted gap:** the seat count-then-create isn't wrapped in a transaction/row lock — a genuine TOCTOU race under real concurrency, same class of tradeoff this repo has accepted elsewhere; revisit if concurrent extension creation ever becomes common enough to matter.

**W2 — landing page (`website/src/content/site.ts`, `page.tsx`).** `pricing` converted from one object to an array; added Premium "Hybrid AI + Human" card, highlighted with existing tokens only.

**W3 — provider registry (`src/lib/ai/providers/**`, `api/admin/ai/providers/**`).** One adapter per provider behind a shared `AiProviderAdapter` interface. Live model-list: OpenAI, Anthropic, Gemini (tags `bidiGenerateContent` models as `realtime`), Groq, Deepgram, ElevenLabs (models+voices), Cartesia, Ultravox, plus a generic `openai_compatible` adapter (OpenRouter/Together/Azure OpenAI/Sarvam-M — just needs the right `baseUrl`). Static catalogs where no list endpoint exists: Sarvam, AssemblyAI, Azure locales, Retell/Vapi (flagged "media via US" — they're SIP-routed platforms, not raw models). `POST` validates a key by actually calling `listModels()` before persisting; the plaintext key is never returned or logged, only `apiKeyCipher` is stored.

**W4 — forked sidecar (`ai-voice-agent/`, new Python 3.11 service, NOT part of `algo-pbx-frontend`).** AudioSocket TCP server (9092) + pre-registration HTTP (9091, upstream's own two-step protocol — kept, not renumbered), 20ms frame pacing + barge-in, provider-pluggable cascade (STT→LLM→TTS: Deepgram/Sarvam/AssemblyAI/Azure STT, OpenAI-compatible/Gemini/Anthropic LLM, Cartesia/Sarvam/Azure TTS) and realtime speech-to-speech (OpenAI Realtime — native μ-law, no resampling; Gemini Live — resamples 8kHz↔16/24kHz via stdlib `audioop` with a linear-interpolation fallback for when `audioop` is removed). AMI-based `handoff.py` transfers a call to a human queue/extension. **Known, accepted gap: `main.py`'s connection handler does not yet instantiate `pipeline.registry` per call** — every provider adapter is built and independently pytest'd, but nothing wires "an audio frame arrived → feed it to a provider → speak the reply" yet. That wiring is the natural next increment before a real AI conversation can happen end-to-end; tracked here so it isn't mistaken for done.

**W5 — Asterisk/compose (`pbx_configs/extensions.conf`, `func_odbc.conf`, `docker-compose.yml`, `ai-voice-agent/Dockerfile`).** New `[from-dinstar-ai]`/`[ai-agent-internal]` contexts (`Answer()`→`AudioSocket(${UNIQUEID},127.0.0.1:9092)`→`Hangup()`); `[from-dinstar]`'s existing `Queue(support_queue,...)` fallback is untouched and still fires whenever the new `AI_INBOUND_EXTENSION()`/`AI_EXTENSION_CHECK()` ODBC lookups (`func_odbc.conf`, fail-closed-to-human, opposite polarity from `RECORDING_ENABLED`'s fail-open) find nothing. `ai-voice-agent` runs `network_mode: host` alongside Asterisk (required to reach 127.0.0.1:9091/9092). **Known, accepted architecture limit (see §2):** this schema/dialplan has no per-DID/per-tenant inbound routing table at all — `AI_INBOUND_EXTENSION()` resolves ONE enabled AI agent for the whole shared Asterisk instance, not per-tenant. Left as an explicit limitation rather than a silent one; W1's agents API now REFUSES to enable a second tenant's AI agent while another tenant's is enabled (409), so the limitation is enforced, not just documented — but only one premium tenant can run AI agents at a time until real per-DID routing is built.

**W6 — internal APIs (`api/internal/ai/**`, `src/lib/ai/sessions.ts`).** `GET agent-config?ext=&tenant=` / `POST sessions`, both gated by a `x-internal-secret` header checked with `timingSafeEqual` against `AI_SIDECAR_SHARED_SECRET`, fail-closed if that env var is unset (mirrors `CDR_INGEST_SECRET`'s existing pattern). Session reports write `AiCallSession` + a best-effort CRM `Activity` (never throws if the CDR hasn't arrived yet — expected, per contract). Left a `checkOutbound()` stub in `compliance.ts` for W8.

**W8 — compliance (`src/lib/ai/compliance.ts`).** Real `checkOutbound()`: agent-disabled check, `outboundEnabled` default-false gate, destination-prefix allowlist (empty = unrestricted, per the schema's own doc comment), midnight-wrap-aware call-hours window, `DoNotCallEntry` lookup, every decision audit-logged. Confirmed (read, didn't guess) that `auth.ts`'s geo-lock block and `/api/me/sip-credentials` are both structurally keyed off a live `User` session and so can never fire for an AI extension (`userId=null`) — no redundant "skip AI" branch added to code that's already provably unreachable for AI. Policy doc comment at the top of the file records the India TRAI (140/1600 series, DND) and UAE TDRA (DNCR, 09:00–18:00, prior consent) reasoning behind `outboundEnabled` shipping `false`.

**W7 — admin UI (`admin/ai-agents/**`, `admin/users`, `admin/settings`, `cdr-table.tsx`, `reports/telephony-tab.tsx`, `billing-tab.tsx`).** Human|AI chooser (gated on `planHasFeature`) + seat meter on the users page; new AI-agent list/editor (provider+model pickers fed by W3's cached model lists); AI-providers CRUD section in settings; AI badge/filter + transcript drawer merged into the EXISTING `cdr-table.tsx`/reports (no separate AI reports page, per the "merge, don't duplicate" instruction); AI usage stat on the platform billing tab. Added 4 API routes not pre-assigned to any node (flagged by the agent, reviewed here): `admin/ai/agents` (list/create), `admin/ai/agents/[id]` (get/patch), `admin/ai/call-sessions` (CDR-drawer lookup), `platform/tenants/[id]/ai-usage`.

**Post-build verification (3 parallel fresh-context reviewers) found real, confirmed bugs — fixed before this merge, not glossed over:**
- **Critical:** the dialplan never called the sidecar's port-9091 pre-registration endpoint before `AudioSocket()`, so `PendingCall` lookup always missed and every AI call sat in dead air. **Fixed:** added `CURL()` calls in both new dialplan contexts, authenticated with the shared secret via `CURLOPT(httpheader)`.
- **Critical:** the sidecar defaulted both its ports to `0.0.0.0` (with `network_mode: host`, reachable from the public internet) and its pre-registration endpoint had NO authentication. **Fixed:** default bind host is now `127.0.0.1`; `/register` now requires the same `x-internal-secret` header, fail-closed if unset.
- **High:** `AI_INBOUND_EXTENSION()` had no tenant scoping (pre-existing architecture limit, see W5 above) — escalated from "routing granularity" to "cross-tenant leak" severity by the reviewer, since it could answer tenant A's caller with tenant B's AI agent identity. **Mitigated** (not fully fixed — the real fix is per-DID routing, out of scope here): app-layer guard now refuses to have more than one tenant's AI agent enabled at once.
- **High:** any tenant admin could point `AiProviderCredential.baseUrl` (the generic `openai_compatible` adapter) at localhost/RFC1918/the cloud metadata address, making the platform's own backend probe internal infrastructure (SSRF) — and the providers POST/refresh routes had no `planHasFeature` gate at all, so this was reachable on ANY plan. **Fixed:** `assertPublicHttpUrl()` guard in the shared fetch helper; plan gate added to the providers POST route.
- **Medium:** the Gemini adapter puts its API key in the request URL's `?key=` query param, and a failed validation call echoed the full URL — including the key — back into the admin API's error response. **Fixed:** `_fetch.ts` strips the query string from every URL before it can appear in a thrown error.
- **Medium:** env var names disagreed between `docker-compose.yml` and the Python code on three variables (audiosocket bind host, pre-reg port, Next.js base URL) — silently masked by matching defaults today, would silently break on any real reconfiguration. **Fixed:** compose renamed to match what the code actually reads.
- **Informational, not fixed (by design, tracked):** `checkOutbound()` is fully built and unit-tested but is not yet called from anywhere, because no code originates an AI outbound call yet (`outboundEnabled` defaults `false`). Whoever builds the actual outbound-dial path next MUST call it at that exact point or the TRAI/TDRA compliance design in `compliance.ts` is decorative — this is a go-live blocker for outbound AI calling specifically, not for the inbound feature merged here.

**Gates before this merge, all green:** `npx prisma generate` clean; `npm run typecheck` clean; `npm run test` — 975/975 passing (87 test files, ~65 new tests across the AI feature); `npm run lint` clean; `npm run build` — clean production build, all new routes in the manifest; `ai-voice-agent`: `python -m pytest tests` — 45/45 passing (39 original + 6 added in the fix round), every module import-checked clean. **Not run/not deployed:** no live Asterisk instance to verify `AudioSocket()`/`CURL()` dialplan syntax against (same "verify against a live `odbc show`/test call before trusting in production" caveat this file already carries for `DNC_CHECK`/`RECORDING_ENABLED`); no `prisma migrate diff` against a shadow DB (no reachable Postgres in the build environment); nothing pushed or deployed to the production VPS. Before enabling this plan for a real tenant: (1) wire W4's `pipeline.registry` into `main.py`'s call handler, (2) place one real inbound test call end-to-end per the plan's verification section, (3) decide the real fix for per-DID AI routing before selling this to more than one tenant concurrently.

**Deployed for real, same day, on user go-ahead.** Committed `668fcdc`, pushed to `origin/main`. VPS `/opt/algo-pbx`: pre-deploy state backed up (commit hash + `git status`) to `/root/pre-crm-vpn-deploy-20260908/`, `git fetch` + `git reset --hard origin/main` (`e67c1fa` → `668fcdc`, clean fast-forward — no divergence to reconcile), confirmed the one known untracked migration dir (`20260904140000_add_tenancy_constrain`) and `website/out` both survived the reset intact. `docker compose build --no-cache web` clean, `docker compose up -d web` — the container's own entrypoint ran `prisma migrate deploy` and applied `20260908120000_add_user_vpn_and_crm_notes` cleanly (32 migrations found, all applied), app ready in 98ms. **Verified**: all 13 containers up, `algo-web` healthy; `/api/health`/`/login`/`/platform/login` 200, `/platform`+`/platform/tenants` 307→login (guard live); apex and `pbx.saharatechs.com` both 200 over HTTPS; zero errors in startup logs; `UserVpnProfile`/`CompanyNote` confirmed present via a direct `psql` query against production, not just inferred from the migration log — the strongest evidence available that the migration is sound, standing in for the shadow-DB diff that wasn't possible pre-deploy. **Not yet done:** a human click-through of the new panels in production (tenant modal, VPN generation, deal/task/note dialogs) — smoke-checked via HTTP status codes only so far.

### 34.1 Per-Dinstar-port AI agent assignment — follow-up to §34, closes the "one AI agent globally" gap at the schema/API/UI layer only (2026-09-14)

§34's post-verification round flagged a real architecture limit: `AI_INBOUND_EXTENSION()` resolved a single enabled AI-kind `Extension` for the whole shared Asterisk instance, so only ONE tenant could run an enabled AI agent at a time (mitigated, not fixed, with a cross-tenant app-layer guard in `POST/PATCH /api/admin/ai/agents`). This pass builds the per-port routing model that lets a single tenant run up to 4 AI agents (one per Dinstar GSM port) — but is honest that the dialplan side of it cannot be verified live yet.

**Schema (`prisma/schema.prisma`, migration `20260914120000_add_ai_agent_dinstar_port`, additive only).** `AiAgent.dinstarPort Int?` — nullable ("not assigned to a GSM port yet", internal-dial-only), 1-4, reusing `WaInstance.simPort`'s exact numbering convention for the same physical Dinstar UC2000 hardware rather than inventing a second scheme. `@@unique([tenantId, dinstarPort])` — Postgres unique indexes treat multiple `NULL`s as non-conflicting, so any number of not-yet-assigned agents coexist freely, while a real port collision on the same tenant is rejected at the DB layer. Migration hand-written (no reachable Postgres in this environment, same constraint every prior session in this repo has had) modeled on `20260914090000_add_hybrid_ai_agents`'s style. `npx prisma generate` clean.

**API (`api/admin/ai/agents/route.ts` + `[id]/route.ts`).** Both `POST` (create) and `PATCH` accept optional `dinstarPort: number (1-4) | null`, zod-validated. Both list/detail `SELECT`s now include `dinstarPort`. A `P2002` on `AiAgent_tenantId_dinstarPort_key` is caught in both routes and turned into `409 "GSM port N is already assigned to another AI agent — unassign it there first."` instead of a raw Prisma error, matching this repo's existing P2002-handling convention (`admin/contacts/route.ts`, `admin/rooms/route.ts`).

**Cross-tenant guard — read carefully, left UNCHANGED, on purpose.** `[id]/route.ts`'s existing "Another tenant already has an AI agent enabled" guard (added post-verification in §34) queries `unsafeGlobalDb.aiAgent.findFirst({ where: { enabled: true, id: { not: params.id }, tenantId: { not: <this agent's tenantId> } } })` — the `tenantId: { not: ... }` filter means it was ALWAYS scoped to cross-tenant collisions only; it never fired for two agents on the same tenant, so two AI agents on two GSM ports for one tenant needed zero changes to this guard. Confirmed by reading it line-by-line, not assumed. Cross-tenant protection is unchanged and unweakened — a second tenant still cannot enable an AI agent while another tenant's is enabled, independent of the new per-port feature.

**UI (`admin/ai-agents/page.tsx` + `admin/ai-agents/[id]/page.tsx`).** List page gained a "GSM port" column (`Port N` / `—`). Editor page gained a "GSM port (Dinstar UC2000)" `Select` in the Basics card with options "Not assigned to a GSM port (internal only)" / "Port 1".."Port 4", wired to the existing autosave-on-save PATCH payload. To show which OTHER agent already holds a port before the admin hits the 409, the editor now also fetches this tenant's full agent list (`GET /api/admin/ai/agents`, already used by the list page) and labels an occupied option `Port N — in use by <agent name>`. `src/components/ui/select.tsx` has no per-option `disabled` support and is outside this pass's owned-files list, so occupied ports are labeled, not disabled — picking one and saving still correctly surfaces the API's 409. Styling matches the existing "Apple black" classes already used in this directory (`glass-card`, `text-secondary`, `border-cyan` focus rings, etc.) — nothing new invented.

**Dialplan (`pbx_configs/func_odbc.conf`, `pbx_configs/extensions.conf`) — built, honestly flagged as NOT live-verified.**
- `func_odbc.conf`'s `[AI_INBOUND_EXTENSION]` now takes `ARG1` (the Dinstar port number) and reads: `readsql=SELECT "Extension"."number" FROM "Extension" JOIN "AiAgent" ON "AiAgent"."extensionId" = "Extension"."id" WHERE "Extension"."agentType" = 'AI' AND "AiAgent"."enabled" = true AND "AiAgent"."dinstarPort" = ${SQL_ESC(${ARG1})} LIMIT 1`. An empty `${ARG1}` matches nothing, falling through to the existing human-queue path — same fail-to-existing-behavior polarity as `RECORDING_ENABLED`/`DNC_CHECK`, unchanged.
- `extensions.conf`'s `[from-dinstar]` now does `Set(DINSTAR_PORT=)` (left deliberately empty, with a long TODO comment) immediately before `Set(AI_INBOUND_EXT=${AI_INBOUND_EXTENSION(${DINSTAR_PORT})})`. **This is the one genuinely unverified piece, called out honestly rather than glossed over:** today all 4 Dinstar GSM ports arrive through one shared Port Group pointed at one shared trunk/AOR (§25.1's "Port Group-0 -> Trunk-0" finding) — there is no live-confirmed per-port signal visible to the dialplan for a call on this shared trunk. A fake extraction (e.g. parsing `CHANNEL(pjsip,remote_uri)` for a port hint that has never been observed to exist) was deliberately NOT written, per the instruction not to invent something that looks confirmed when it isn't. Making this real requires one of: (a) reconfiguring the gateway's `PortCfg`/`SipAccN`/`AuthenticateIDN` per port (§25.1's mapped HTTP API, `POST /goform/PortCfg`, ports 0-7 in the gateway's own 0-indexed API — i.e. `dinstarPort - 1`, the exact same off-by-one convention `src/lib/messaging/dinstar-sms-provider.ts`'s `simPortToApiPort()` already uses for the WhatsApp/SMS side) so each port registers as its own distinct SIP identity and lands in Asterisk via a distinct endpoint/context instead of one shared trunk, or (b) a live trace of what the gateway actually sends per port on the CURRENT shared-trunk config to see if an unused port hint already exists. Neither has been done.
- `[AI_EXTENSION_CHECK]` was deliberately left unchanged (port-unaware) — a comment now explains why: `[ai-agent-internal]`'s internal-dial path (an already-authenticated PJSIP agent dialing an AI extension's own number) never touches the Dinstar gateway or a GSM port at all, so there is no port concept to add there.

**Verification, all green in this environment:** `npx prisma generate` clean; `npm run typecheck` clean; `npm run test` — 978/978 passing (87 test files; 3 new tests added to `api/admin/ai/agents/route.test.ts` covering `dinstarPort` create-time default/pass-through, range validation, and the P2002→409 mapping). **Not run/not verified, same honest gaps as §34 itself:** no live Asterisk instance to confirm the new `AI_INBOUND_EXTENSION(${ARG1})` syntax or that `DINSTAR_PORT` can ever be populated for real; no `prisma migrate diff` against a shadow DB (no reachable Postgres in this build environment); nothing pushed, deployed, or committed. Before this feature can route a real call to a real per-port agent: reconfigure and live-trace the Dinstar gateway per the two options above, then place one real inbound test call per port and confirm `AI_INBOUND_EXT` resolves the intended per-port agent in the Asterisk CLI.

### 34.2 AI → human escalation, Workstream A only (LLM tool-calling) — a much larger 7-workstream plan started, one workstream built and tested, the rest NOT started (2026-09-14, same day, follow-up to §34/§34.1)

Owner requirement: when an AI agent's caller insists on a human, the AI must hold the call, dial a phone number configured for that agent **out over the GSM trunk from its own extension** (not an extension transfer), merge into a conference, and stay in the call until both parties hang up. If no human answers, offer wait-or-callback.

Full design plan (7 workstreams: A–tool-calling, B–state machine/session continuity, C–Next.js AMI orchestration, D–Asterisk dialplan/ConfBridge, E–CRM callback, F–schema/API/UI, G–compliance) is saved at `~/.claude/plans/we-have-succeesfully-forked-crispy-petal.md` — read it before continuing this work, it is the source of truth for B–G, not restated here.

**Critical finding from the design pass, gating the whole feature:** `src/lib/transfer-guard.ts:1-14` records a **live-confirmed SIP trace** — an agent on an active GSM call tried a second outbound call through the same Dinstar trunk and got a 503 (single active SIM/port, no spare channel). Confirmed with the owner: **only 1 SIM is active today.** So AI→GSM escalation cannot work for an inbound-GSM caller until more SIMs are active (which also depends on §34.1's unfinished per-port routing) — it can only work today for internal/WebRTC-originated calls, or when the target is an internal extension instead of an external number. Workstream C's design includes an explicit `GSM_TRUNK_CAPACITY`-gated capacity guard for exactly this reason — build that guard before wiring the GSM dial path live.

**What was actually built and tested this session — Workstream A only (LLM tool-calling in the Python sidecar):**
- `pipeline/base.py`: added `ToolSpec`, `ToolCall`, `LlmDelta{text, tool_call}`. Changed `LlmProvider.generate(messages, tools=None) -> AsyncIterator[LlmDelta]` (was `AsyncIterator[str]` — a breaking signature change, see below) and added `tools`/`on_tool_call` params to `RealtimeProvider.run()`.
- `pipeline/tools.py` (new): the single `request_human_handoff` `ToolSpec` (`reason`, `caller_intent`), shared by both CASCADE and REALTIME so escalation behaves identically regardless of pipeline mode.
- `pipeline/providers/openai_compatible.py`: sends `tools` in the request body; accumulates streamed `delta.tool_calls[]` fragments per index (id/name arrive first, `arguments` streams as JSON text fragments) and yields a completed `ToolCall` on `finish_reason == "tool_calls"`. Malformed JSON in accumulated arguments degrades to `{}` rather than raising.
- `pipeline/providers/gemini.py`: `GeminiLlm.generate` now sends `tools:[{functionDeclarations}]` and parses `parts[].functionCall` vs `parts[].text`. `GeminiRealtime.run` gained `tools`/`on_tool_call` params, sends `tools` in the `setup` message, and dispatches `toolCall.functionCalls[]` to `on_tool_call`.
- `pipeline/providers/openai_realtime.py`: `session.update` now includes `tools`+`tool_choice:"auto"` in OpenAI Realtime's **flat** tool shape (name/description/parameters at the top level — contrast with Chat Completions' nested `{type:"function",function:{...}}` in `openai_compatible.py`). Accumulates `response.output_item.added` (type `function_call`, carries name/call_id keyed by `item_id`) → `response.function_call_arguments.delta` (JSON fragments) → `...done` (fires `on_tool_call`).
- `pipeline/runner.py`: `PipelineRunner` gained an `escalation_request: ToolCall | None` field and a shared `_handle_tool_call()` used by both `_run_cascade` (routes `LlmDelta.tool_call` there instead of into spoken reply text, and breaks the turn loop once set) and `_run_realtime` (passed as `on_tool_call`). This is the seam Workstream B's `escalation.py` state machine will consume — **not built yet**, see below.
- `main.py`: after `runner.stop()`, if `escalation_request` is set it is **logged, not acted on** — deliberately does not report `outcome="handed_off"`, since nothing yet performs the actual AMI hold/dial/merge (that outcome must mean a human actually joined, not merely that one was requested). The hardcoded `outcome="completed"` in `_report_session` is UNCHANGED this session.
- `handoff.py` (the AMI `redirect_to_extension` mirror) was **not touched or deleted** — the plan recommends deleting it once Workstream C lands (Next.js owns AMI, confirmed by `docker-compose.yml:510-524` never setting `AMI_*` env vars for this container), but that's a follow-up decision, not done here.

**Tests**: `tests/test_tool_calling.py` (new, 5 tests) — OpenAI-compatible tools-payload shape, streamed tool-call accumulation, malformed-argument tolerance, Gemini `functionCall` parsing, Gemini plain-text-has-no-tool-call. Extended `tests/test_pipeline_runner.py` with `test_cascade_tool_call_sets_escalation_and_never_speaks_it`. Fixed the two tests broken by the `LlmProvider.generate` signature change (`test_provider_openai_compatible.py`, `test_pipeline_runner.py`'s `FakeLlm`). **`ai-voice-agent` suite: 55/55 passing** (49 prior + 6 new). Frontend gates not re-run this session — nothing in `algo-pbx-frontend/` was touched.

**NOT started at the end of that session, per the plan's own suggested build order (items 3–7): Workstream F (schema/API/UI — no `AiAgent.escalationEnabled`/`handoffNumberE164`/`handoffExtensionId` columns exist yet), Workstream D (no `[ai-conference]`/`[ai-conference-leg]`/`[ai-queue-fallback]` dialplan contexts, no `confbridge.conf` profiles), Workstream C (no `src/app/api/internal/ai/escalate/route.ts`), Workstream B's actual state machine (`escalation.py` doesn't exist — only the runner-level `escalation_request` seam it will consume), Workstream E (no CRM callback wiring), Workstream G (no `checkEscalationDial()`).** The plan's own risk list flags the ConfBridge-merge mechanism (`Local/…/n` + `AudioSocket`) as the single most load-bearing, hardware-unverifiable assumption in the whole feature — test that first once Workstream D/C are built, per the plan's suggested order.

### 34.3 Workstream F (schema/API/UI) built and gated green — same-day follow-up to §34.2

**Schema** (`prisma/schema.prisma`, migration `20260914130000_add_ai_agent_handoff`, additive only). `AiAgent` gains `escalationEnabled Boolean @default(false)` (ships off, same polarity as `outboundEnabled`/`dinstarPort`), `handoffTargetKind String?` (`"NUMBER" | "EXTENSION"`), `handoffNumberE164 String?`, and `handoffExtensionId String?` with a named relation `handoffExtension` to `Extension` (`ON DELETE SET NULL`, matching this schema's existing convention for every other optional FK, e.g. `Contact.companyId`) — a **named** relation because `Extension` already has an unnamed 1:1 back-relation to `AiAgent` for its own extension; this is a separate, optional many-to-one (many agents may point at the same human extension). `Extension` gains the back-relation array `aiHandoffTargets`. `npx prisma generate` clean.

**`agent-config/route.ts:76-104`** now resolves `handoffExtensionHint` instead of the old hardcoded `null`: non-null **only** when `escalationEnabled` is true, giving `handoffNumberE164` for a `"NUMBER"` target or the linked extension's number for `"EXTENSION"` — an agent with escalation off, or enabled with no target picked yet, correctly gets `null`. No contract change (`AiAgentConfigResponse.handoffExtensionHint` was already `string | null`).

**`ai-voice-agent/pipeline/runner.py`** — closed a gap the plan's own Workstream A left open: `PipelineRunner._run_cascade`/`_run_realtime` previously advertised `request_human_handoff` **unconditionally to every agent**, regardless of whether escalation was configured. Added `_escalation_tools()`, gated on `config.handoff_extension_hint` — an agent with escalation off (or on with no target) is never given the tool at all, not just has its use silently ignored. 2 new sidecar tests; **`ai-voice-agent` suite now 57/57.**

**`agents/[id]/route.ts`** — `AGENT_DETAIL_SELECT` and `UpdateAgentSchema` extended with all four fields. New validation: `handoffNumberE164` is normalized/validated through `normalizeToE164()` (`src/lib/phone-normalize.ts` — the same single source of truth the DNC importer and `checkOutbound()` already use, not a new regex) rather than trusted as typed; `handoffExtensionId` must resolve, within this tenant-scoped `db`, to an `Extension` with `agentType === "HUMAN"` — mirroring the existing `CREDENTIAL_ID_FIELDS` cross-tenant/typo check in the same file. New test file `[id]/route.test.ts` (10 tests — this route had **no test file at all** before this session, a real pre-existing gap now closed) plus the escalation-hint tests added to `agent-config/route.test.ts` (4 new) and a new `sessions/route.test.ts` (8 tests, also previously missing).

**`sessions/route.ts`** — `SessionReportSchema` gained a `.refine()`: `handoffExtensionId` may only be non-null when `outcome === "handed_off"`. Deliberately does **not** require it to resolve to any Extension row — `AiCallSession.handoffExtensionId` has no FK by design (it may hold an external E.164 number dialed over GSM, not just an internal extension number), so the only enforceable rule is the outcome pairing.

**`src/app/api/extensions/route.ts`** — added `agentType` to the existing `GET` select (additive, no behavior change) so the new UI picker below can filter to HUMAN extensions without a second endpoint.

**`src/app/admin/ai-agents/[id]/page.tsx`** — new "Escalation to a human" `glass-card`, matching every existing convention in this file (the `"none"`-sentinel `<Select>` pattern, CSS-var/semantic classes, explicit `aria-label`s, a `border-warning/40 bg-warning/10` box for the risky case — here, that a `NUMBER` target will fail against the single active SIM's capacity, stated plainly rather than hidden). Fields added in the same four places every prior field needed: the `AgentDetail` interface, the PATCH body literal, the JSX, and the API schema.

**Gates, all green:** `npx prisma generate` clean; `npm run typecheck` clean; `npm run test` — **1000/1000 passing** (89 test files, 22 new); `npm run lint` clean; `npm run build` — clean production build. `ai-voice-agent`: `python -m pytest tests` — 57/57. **Not run, same standing constraint as every prior session:** `prisma migrate diff`/`migrate deploy` against a real Postgres (none reachable in this environment) — the migration was hand-written and modeled on `20260914120000_add_ai_agent_dinstar_port`'s style, not diff-verified.

**Still NOT started at the end of that session: Workstream B's actual `escalation.py` state machine, Workstream C (`escalate/route.ts` — no AMI orchestration exists), Workstream D (no dialplan/ConfBridge changes), Workstream E (no CRM callback wiring), Workstream G (no `checkEscalationDial()`).** Nothing here was committed.

### 34.4 Workstreams C+D (escalate API + Asterisk config) built, gated green, one real architecture correction found mid-implementation, NOT live-tested — same-day follow-up to §34.3

**A real infrastructure bug found and fixed incidentally, unrelated to escalation specifically:** `docker-compose.yml`'s `web` service never set `AI_SIDECAR_SHARED_SECRET` in its own `environment:` block — only the `ai-voice-agent` service had it. `isAuthorizedInternalAiRequest()` (`src/app/api/internal/ai/_auth.ts`) fails closed on an unset env var, so **every request the sidecar has ever sent to `/api/internal/ai/agent-config` and `/api/internal/ai/sessions` would 401 in this compose config** — a pre-existing bug from §34's original build, not introduced this session, caught only because this session needed to reason carefully about that same variable. Fixed: added the line to `web`'s block (matches the `ai-voice-agent` service's own value). Also added `GSM_TRUNK_CAPACITY` (default `1`) to both `.env.example` and `web`'s environment — the new capacity guard below reads it.

**A real architecture correction found while implementing, not in the original plan draft:** the plan (`~/.claude/plans/we-have-succeesfully-forked-crispy-petal.md`, Workstream C step 3) originally had Next.js POST directly to the sidecar's own `127.0.0.1:9091/register` before Originating the AI's conference leg. **This is unreachable** — `web` runs on the bridge network (`algo-net`), not `network_mode: host` like `ai-voice-agent`/`asterisk`, and the sidecar's pre-registration server is deliberately bound to `127.0.0.1` only (a §34 post-verification security fix, so widening it to `0.0.0.0` was correctly rejected as reopening a public-internet exposure). `web` reaches Asterisk's AMI fine via `host.docker.internal:5038` because `manager.conf` listens broadly, but there is no equivalent reachable address for a loopback-only HTTP server on a container that isn't host-networked. **Fix**: the new `[ai-conference-leg]` dialplan context does its own `CURL()` pre-registration — the exact same mechanism `[ai-agent-internal]`/`[from-dinstar-ai]` already use and have proven reachable, since Asterisk itself is host-networked same as the sidecar. Everything the CURL() body needs (the new leg's UUID, which call it resumes, the tenant, the AI's own extension number) is threaded through as channel variables on the AMI `Originate` action's `Variable:` field instead of an HTTP call Next.js can't make. `pbx_configs/extensions.conf`'s `[ai-conference-leg]` context and its long header comment carry the full reasoning.

**`pbx_configs/extensions.conf`** — three new contexts appended after `[from-dinstar-ai]`, exactly per the plan: `[ai-conference]` (`_9XXXXXX` → `Answer()` → `ConfBridge(${EXTEN},ai_conf_bridge,ai_conf_marked)`, no MixMonitor — the caller's recording already started upstream and follows the Redirect), `[ai-conference-leg]` (the AI's own media leg — `Set(TIMEOUT(absolute)=3600)` → CURL pre-registration per the correction above → `Answer()` → `AudioSocket(${AI_CONF_UUID},127.0.0.1:9092)` — `Answer()` before `AudioSocket()` is load-bearing, not cosmetic: a dialplan Originate's `;1` half isn't delivered to its Context/Exten until `;2` actually answers), and `[ai-queue-fallback]` (the missing bare "enter the queue" extension — re-Redirecting into `[from-dinstar],s` would start a second `MixMonitor` on an already-recording channel).

**`pbx_configs/confbridge.conf`** — three new profiles appended, `[default_bridge]`/`[default_user]` left untouched (manager-merge/conference API routes depend on them): `[ai_conf_bridge]` (`mixing_interval=20`, `internal_sample_rate=8000` — every leg in this feature is 8kHz), `[ai_conf_marked]` (caller + human, `marked=yes`), `[ai_conf_bot]` (the AI's own leg, `marked=no`/`end_marked=yes`/`wait_marked=no`/`quiet=yes` — `end_marked` is the entire auto-teardown mechanism: it runs inside Asterisk, survives a Next.js restart, and needs no AMI event-listener holding open for the call's whole duration).

**`src/app/api/internal/ai/escalate/route.ts`** (new). Two actions, both stateless/re-derive-everything (a `merge` call never trusts a prior `check` call was authoritative — there is no session between the two HTTP requests):
- `check` — resolves the agent's configured target (`escalationEnabled` → `handoffTargetKind` → the matching field), resolves the caller's Asterisk channel via `CoreShowChannels` filtered on `Uniqueid === callUuid` (the AI leg **is** the caller's channel, so this equality is exact — confirmed by the design pass, no dialplan `${CHANNEL}` needed), and for a `NUMBER` target counts active `PJSIP/dinstar-trunk-` channels against `GSM_TRUNK_CAPACITY` (default 1).
- `merge` — the ordered sequence per the plan, inverted from `manager-merge`'s own order specifically so the caller is never moved into an empty, silent room: Originate the AI's own leg first (`Local/ai@ai-conference-leg/n`, `Codecs: slin`, tenant/uuid/extension threaded via `Variable:`) → gate on a `ConfbridgeJoin` event (8s; resolves `null` on timeout, not a throw) → only then `Redirect` the caller in → Originate the human (`Local/<number>@from-agent-<tier>/n` for a `NUMBER` target — **never** `PJSIP/<n>@dinstar-trunk`, same rule `api/calls/conference/route.ts` already established; `PJSIP/<ext>` for an `EXTENSION` target) → observe `OriginateResponse`/`Exten` for the answer outcome.
- **One more implementation-level correction from the plan's Workstream C bullet 8**: the plan said to reuse `src/lib/escalation.ts`'s `watchEscalationOutcome()` (keyed on a `PJSIP/<ext>-` `DialEnd` event) to classify the human's answer. That does **not** apply here — `DialEnd` is emitted by the `Dial()` application, and this route's human Originate targets a dialplan `Context`/`Exten` directly (landing in `ConfBridge()`), the same shape `api/calls/manager-merge/route.ts`'s own manager-Originate already uses. Followed that route's `OriginateResponse`/`Exten` pattern instead — an honest "doesn't actually fit" correction of the same kind `conference-orchestration.ts`'s own header already models for this codebase.
- **Also deliberately deviated from the plan's "fire the human Originate concurrently with the Redirect" latency optimization** — implemented sequentially (Redirect awaited, then Originate) so a caller who vanishes between the join-gate and the Redirect can never result in a human being Originated into an AI-only room with nobody there. Costs roughly 1s of extra ring time; judged a better trade than the race.
- Tenant is **never** accepted from the sidecar's request body — always re-resolved from `agentId` via `unsafeGlobalDb`, per the plan's own risk item 7 (the dialplan's `AI_TENANT_ID` is empty today, so the sidecar cannot be trusted to know its own tenant).

**Tests**: `src/app/api/internal/ai/escalate/route.test.ts` (new, 17 tests) — auth fail-closed, every `check`/`merge` unavailable/failure reason (`agent_not_found`, `escalation_disabled`, `no_target_configured`, `call_not_found`, `gsm_capacity`, `ai_leg_join_timeout` with the stray-leg reap, `caller_hung_up` with the captured-channel Hangup and confirmation the human is never dialed), the full merge happy path for both target kinds, and an explicit assertion that a `NUMBER` target's Originate channel is `Local/…@from-agent-international/n` and never matches `/dinstar-trunk/`.

**Gates, all green:** `npm run typecheck` clean; `npm run test` — **1017/1017 passing** (90 files, 17 new); `npm run lint` clean; `npm run build` — clean production build, `/api/internal/ai/escalate` in the route manifest. `ai-voice-agent`: 57/57 (unchanged this session — no Python files touched).

**NOT done, not glossed over:**
- **Not live-tested against a real Asterisk instance — this is the single most load-bearing, hardware-unverifiable piece of the entire feature**, exactly as the plan's own risk list flagged. Every AMI field name/event shape here (`ConfbridgeJoin`'s exact fields, `OriginateResponse`'s field set for a `Local/` channel target, whether a dialplan Originate's `;1` half really does wait for `;2` to answer before delivery, whether `Codecs: slin` is honored on a Local Originate) is reconstructed from Asterisk 20 documentation and this codebase's own prior "MEDIUM-LOW confidence, needs live testing" precedent (`conference/route.ts`'s own header), not observed. **Before this is trusted**: place one real internal-extension-target escalation call (needs no GSM capacity) and confirm the join gate actually fires, the caller is never silent, and `end_marked` actually tears the AI leg down when both other parties hang up.
- **Workstream B is still not built** — the sidecar's `[ai-conference-leg]` CURL() now sends `resume_of`/`role` fields, but `main.py`'s `handle_register`/`PendingCall` don't read them yet (Python dict access on unknown keys is silently a no-op, so nothing breaks — but nothing resumes the original session either; a live merge today would register the conference leg as if it were a brand-new call with no conversation history). This is exactly Workstream B's job, next per the plan's build order.
- **Workstream E (CRM callback) and G (`checkEscalationDial()` compliance gate) are still not built.**
- **`GSM_TRUNK_CAPACITY` is a fixed env var, not per-tenant** — every tenant on this Asterisk instance shares the same physical Dinstar trunk, so a global capacity setting is correct for now, but note it if per-tenant Dinstar hardware is ever introduced.

### 34.5 Workstream B (state machine + session continuity) built and tested — same-day follow-up to §34.4

**`ai-voice-agent/escalation.py`** (new). `EscalationClient` — thin `httpx` POST wrapper for `/api/internal/ai/escalate`, mirroring `config_client.py`/`session_reporter.py`'s own shape (same env vars, same `x-internal-secret` header). `EscalationController.request_handoff(reason)` drives the flow: speaks "let me get a colleague, one moment" → `check` → speak an apology and stop if unavailable → `merge` → speak nothing further on success (the caller has already been AMI-Redirected out of this connection by the time the HTTP call returns) or an apology on failure. `speak` is an injected async callback rather than this module touching TTS directly, which is what makes it fully unit-testable against a fake client with no real HTTP or audio involved. Per-reason caller-facing messages are a dict (`_UNAVAILABLE_MESSAGES`), not an if/elif chain, so an unrecognized reason from a future Next.js change degrades to a generic message instead of a `KeyError` taking down the call.

**Two gaps stated openly, not silently worked around**, both in the module's own docstring:
1. **No interactive "wait or callback" sub-dialogue.** Workstream E (the CRM callback) doesn't exist, so there is nothing for a spoken "would you like a callback?" to actually do once answered — asking without the mechanism behind it would be worse than not asking. Today the AI apologizes once and the conversation simply continues (see `pipeline/runner.py` below); the LLM's own system prompt is free to offer a retry in its next turn, but that's model behavior, not a scripted node.
2. **REALTIME mode cannot narrate its own progress.** `speak()` is a no-op there (see `pipeline/runner.py`) — a realtime vendor session owns its own audio pipeline end-to-end, and neither the OpenAI Realtime nor Gemini Live adapter exposes a "say exactly this text" hook independent of the model's own turn-taking. The check/merge HTTP calls and the AMI-side merge itself work identically in REALTIME mode; only the spoken narration is missing. Also not implemented: replying to the vendor's own tool-call protocol (e.g. OpenAI Realtime's `function_call_output`) so the model itself knows the call resolved.

**`pipeline/runner.py`** — `PipelineRunner` gained `call_uuid` (needed for the `/escalate` calls), `resume_context`/`skip_greeting` (set only by a resumed connection, main.py below), and `escalation_outcome`. `_handle_tool_call` now actually constructs an `EscalationController` and drives it, instead of only logging. **A real behavioral bug fixed in the same pass**: the cascade turn loop used to unconditionally `break` after ANY escalation attempt, success or failure — meaning a *failed* transfer silently ended the AI's participation in the call even though the caller was still on the line. Now it only breaks on an actual merge (`escalation_outcome.merged`); on a non-merge it clears `escalation_request`/`escalation_outcome` and the conversation continues normally, so a later attempt is possible and the call never just goes quiet.

**`ai-voice-agent/main.py`** — the full session-continuity design from the plan, built as specified:
- `PendingCall` gained optional `resume_of`/`role` (`.get()`'d, so the existing `[ai-agent-internal]`/`[from-dinstar-ai]` CURL() bodies are unaffected).
- New `LiveSession` dataclass (`config`, `transcript`, `original_call_uuid`, `migrated_to`, `handoff_target_label`, `ready_event`), stored in `AudioSocketServer._sessions` keyed by `original_call_uuid`, **never re-keyed** — both the original and any resumed leg address it by that same key.
- A resumed connection reuses `config`/`transcript` from the found `LiveSession` (skips `config_client.fetch()` and the greeting entirely), waits up to 10s on `ready_event` (set by the original leg's own `finally`) before starting its runner — this is what stops the two connections' TTS/audio from ever overlapping — and seeds a synthetic system message telling the model it's now in a 3-way conference.
- Reporting ownership is exactly one leg, never both, never neither: the resumed leg always reports (`outcome="handed_off"`, `cdr_unique_id` = the *original* uuid, `handoff_extension_id` = the merge's target label); the original leg reports normally UNLESS `migrated_to` got set, in which case it skips entirely. `_sessions` entries are cleaned up by whichever leg is terminal.
- `PendingCall.registered_at`'s pre-existing leak (set, never read) is now closed: a `_reap_stale_registrations()` background task drops any `/register` entry older than 60s that never got an AudioSocket connection — this became load-bearing rather than theoretical once `escalate/route.ts` started pre-registering a conference leg *before* an `Originate` that can fail.
- `handoff.py` (the old unused AMI `redirect_to_extension`) was **left alone again this session** — still recommended for deletion once nobody objects, still not actually removed.

**Tests**: `ai-voice-agent/tests/test_escalation.py` (new, 9 tests) — the full `EscalationController` flow against a fake `EscalationClient` (announce-line-first, each unavailable reason's message, the unknown-reason fallback, `caller_hung_up` speaking nothing further, check/merge request failures) plus `EscalationClient`'s own HTTP shape (shared-secret header, action body, non-200 handling) against a faked `httpx.AsyncClient`. `ai-voice-agent/tests/test_session_resume.py` (new, 4 tests) — drives `AudioSocketServer._handle_connection` directly against fake reader/writer objects with `PipelineRunner` monkeypatched to a bookkeeping fake: a normal call creates and cleans up its own session; **two connections run as real concurrent asyncio tasks with the ordering controlled by queue-backed fake readers**, proving the original leg genuinely skips its own report once `migrated_to` is set and the resumed leg reports exactly once under the original uuid with the right outcome/handoff target; a `resume_of` pointing at a session that no longer exists falls back to a fresh fetch instead of failing the call; the resumed leg's `PipelineRunner` is constructed with the literal same transcript list object the original leg was writing to. Extended `tests/test_pipeline_runner.py`: the old test asserting `escalation_request` survives a call is now split into a merge-succeeds test (asserts `escalation_outcome.merged`) and a new blocked-then-continues test (asserts the state is cleared and a second utterance is handled normally afterward) — both monkeypatch `pipeline.runner.EscalationController` to a fake rather than hitting real HTTP.

**Gates, all green:** `ai-voice-agent`: **71/71** (57 prior + 9 + 4 + net +1 from the split test). Frontend: **1017/1017** unchanged — no `algo-pbx-frontend/` files touched this pass, confirmed by re-running the full suite anyway.

**NOT done, not glossed over:**
- **Still not live-tested** — same standing caveat as §34.4. Nothing about session continuity changes that; if anything it adds more surface (the `ready_event` timing, the CURL() pre-registration racing against the AMI Originate) that can only be truly confirmed against real Asterisk.
- **No interactive wait/callback dialogue and no REALTIME narration** — both stated above, both deliberate scope cuts pending Workstream E and a REALTIME text-injection mechanism respectively, not oversights.
- **No chained second escalation** — a resumed leg is architecturally terminal (its own `finally` always cleans up `_sessions`); if a caller escalates, gets a human, and that human tries to escalate again, there is no session to resume into. Out of scope for this pass.
- **Workstream E (CRM callback) and G (`checkEscalationDial()` compliance gate) are still not built** — same as before.

### 34.6 Workstreams G+E (compliance gate + CRM callback) built and tested, plus the callback offer is a real interactive tool now, plus one of the two flagged CDR side-effects closed — same-day follow-up to §34.5, ALL SEVEN WORKSTREAMS NOW BUILT

**Workstream G — `checkEscalationDial()`** (`src/lib/ai/compliance.ts`, new). Deliberately a **separate** gate from `checkOutbound()`/`outboundEnabled`, not a reuse — the module comment explains why at length: `outboundEnabled` guards against an unattended agent working an LLM-driven call list at scale (the TRAI/TDRA "automated marketing" pattern `checkOutbound()`'s own header describes); an escalation dial is the opposite shape, one call to one fixed admin-configured number because a specific caller asked for a human right now. Gated on `escalationEnabled` instead. **Deliberately skips the call-hours window** — a caller asking to be connected right now is not the unsolicited-contact case call-hours exists to stop; whether staff are actually available is `escalate/route.ts`'s own `check` action's job (GSM capacity, `OriginateResponse`), not a regulatory-hours gate. Keeps the destination allowlist and DNC check, audited under a distinct `ai.escalation_compliance_check` action so it's never conflated with an ordinary outbound-dial decision.

**Wired in for real, not just built-and-unused**: `escalate/route.ts`'s new `checkCompliance()` helper calls it for a `NUMBER` target in both `check` and `merge`, before anything touches AMI — a denial returns `reason: "compliance_denied"`, handled by the sidecar exactly like a GSM-capacity refusal. `escalationEnabled` was already checked earlier in `resolveEscalationTarget`, so this call adds genuinely new protection: `allowedDestinations` enforcement (not checked anywhere else in the escalate path before this) plus DNC as defense-in-depth alongside the dialplan's own `DNC_CHECK()` (which already runs unconditionally on the `Local/<n>@from-agent-<tier>/n` route regardless of this check). **Still needs explicit owner sign-off before a real tenant's `escalationEnabled`+`NUMBER` combination goes live** — the code being wired is not the same as the business judgment being approved; record that sign-off here when it happens.

**Workstream E — CRM callback** (`escalate/route.ts`'s new `callback` action). Resolves the caller's number from the same `CoreShowChannels` lookup `check`/`merge` already use (`CallerIDNum` field — same "reconstructed from docs, not observed live" confidence tier as every other AMI field mapping in this codebase), find-or-creates a `Contact` by `tenantId_numberE164` (an unknown GSM caller has none today), and calls `src/lib/crm/tasks-data.ts`'s existing `createTask()` as-is — no new CRM plumbing, it already writes the matching unified-timeline `Activity` row. **Assignee resolution is never null** (`ContactTask.assigneeId` is a required FK): the handoff extension's linked `User` first, a tenant admin (`role: "ADMIN"`) as fallback. The AI has no `User` row to be the Activity's actor, so `ensureSystemActorId()` is reused — the same per-tenant system-actor pattern `checkOutbound()`/`checkEscalationDial()` already use for the identical problem.

**A real gap closed while building this, going beyond what the plan's Workstream B sketch called "no interactive wait/callback sub-dialogue" and leaving it open**: rather than trying to script-parse a caller's freeform spoken answer to "would you like to wait or get a callback?", `pipeline/tools.py` gained a **second** tool, `request_callback` — offered only *after* `request_human_handoff` has already failed once (`PipelineRunner.handoff_failed_once`, a flag that persists for the call's lifetime, unlike the per-attempt `escalation_request`/`escalation_outcome` which reset after each try). The model itself decides when the caller has agreed to a callback (it already has to understand that answer to stay coherent) and calls the tool; `EscalationController.request_callback()` drives it through the same HTTP path. Every apology message (`escalation.py`'s `_UNAVAILABLE_MESSAGES`) now explicitly invites the offer, since a tool being *available* to the model doesn't mean the caller hears it mentioned.

**One of the plan's two flagged side effects closed**: `src/lib/cdr-mapper.ts`'s `mapCdrEventToIngestPayload()` now filters out the AI conference leg's own phantom `Cdr` event (`Local/ai@ai-conference-leg-*`, checked on both `Channel` and `DestinationChannel` since which half of the Local pair reports which field is unverified) — returns `null`, same as an unmappable event, so `ami-cdr-listener.ts` already skips posting it, no listener changes needed. **The other flagged side effect — wallboard concurrency counting a 3-way escalation as more channels than a normal call — is deliberately left as a documented risk, not fixed**: unlike the CDR case, there's no clearly-correct answer here without knowing what "concurrent calls" should mean for this metric once a call legitimately spans 3 channels, and the plan itself didn't prescribe one; forcing a fix now risks encoding a wrong assumption. Revisit once the feature is live and someone can look at a real wallboard during a real escalation.

**Tests**: `src/lib/ai/compliance.test.ts` extended (+6, `checkEscalationDial` — agent-not-found, escalation-disabled and NOT outbound-gated, allowedDestinations, DNC, allow-path, distinct audit action). `escalate/route.test.ts` extended (+6 for compliance-denial on both actions, +6 for `callback` — every failure reason, the success path with Contact upsert + assignee resolution, the tenant-admin fallback). `ai-voice-agent/tests/test_escalation.py` extended (+5, `request_callback`'s full flow). `ai-voice-agent/tests/test_pipeline_runner.py` extended (+2, the callback-tool-only-offered-after-a-failure gate, and a full cascade run driving handoff-fails-then-callback-succeeds end to end). `cdr-mapper.test.ts` extended (+3, filters by `Channel`, filters by `DestinationChannel`, does NOT false-positive on an unrelated channel name containing "ai").

**Gates, all green:** `ai-voice-agent`: **78/78** (71 prior + 7 new). Frontend: `npm run typecheck` clean, `npm run test` **1035/1035** (90 files, 21 new), `npm run lint` clean, `npm run build` clean production build.

**This closes all seven workstreams of the AI → human escalation plan** (`~/.claude/plans/we-have-succeesfully-forked-crispy-petal.md`) at the code level. **What remains before any of this touches a real tenant, stated exactly as plainly as every other gap in this section:**
1. **Still not live-tested against real Asterisk — unchanged since §34.4, still the single biggest open risk.** Nothing built in G/E changes this; if anything E adds one more AMI-dependent path (`CoreShowChannels`' `CallerIDNum` field) to the pile of "reconstructed from docs, not observed" assumptions.
2. **Workstream G's compliance judgment still needs explicit owner sign-off** before `escalationEnabled`+`NUMBER` is ever turned on for a real tenant.
3. **`handoff.py` (the old unused sidecar AMI client) is still sitting there, still unused, still not deleted** — flagged in every session since Workstream A without anyone confirming the deletion.
4. **The wallboard concurrency-counting side effect is a known, documented, unfixed risk**, not an oversight.

Nothing here was committed.

## 35. Owner plan upgrade/downgrade + independent caller-routing feature (Tel-Agent gap analysis) — both built and gated green, same-day follow-up to §34's escalation work (2026-09-14)

Two unrelated features built in the same session, both complete:

### 35.1 Owner plan upgrade/downgrade

**A plan-change action already existed** (`PATCH /api/platform/tenants/[id]/billing`, `action: "change_plan"`) — this closes the three real gaps it left open, per the owner's explicit decisions: AI stays on **premium only** (not moved to pro); a downgrade below extensions-in-use is **blocked** with a clear error; a downgrade off the AI feature **locks** (not deletes) the tenant's `AiAgent` rows; seats auto-fill to the new plan's ceiling in the UI.

**`src/lib/platform/plan-catalog.ts`** — added an explicit `rank` to `PlanDefinition` (price and seat-ceiling order already agree across the whole catalogue: trial 0 < standard 1 < premium 2 < pro 3). New pure functions: `comparePlans(from, to)` → upgrade/downgrade/same, and `describePlanChange({fromPlanId, toPlanId, newSeats, extensionsInUse, aiAgentCount})` → `{direction, priceDeltaUsd, seatCeilingDelta, featuresGained, featuresLost, blockers, aiAgentsToLock}`. **One deliberately-accepted wrinkle, tested explicitly**: `premium → pro` ranks as an *upgrade* (more price, more seats) while losing `aiAgents` — the seat/feature checks run on every change regardless of direction, so this case is still caught and surfaced, just labelled "upgrade."

**`billing/route.ts`** — before the write: counts `Extension`/`AiAgent` rows, calls `describePlanChange`, 409s with the exact blocker message if seats would go below extensions in use. Inside the same `$transaction` as `tenant.update`: `aiAgent.updateMany({enabled: false})` when the destination plan loses `aiAgents` and the tenant has agents. **The route's own header forbids touching telephony** (a hard rule, with strong reasoning) — this is the one named, documented exception: locking `AiAgent.enabled` does change what `AI_INBOUND_EXTENSION()`'s ODBC lookup resolves (a locked agent's calls fall through to the human queue, the same safe failure mode that function already uses for "no match"), so it's audited with `telephonyAffected: true` specifically for this branch, never silently folded into the `false` every other billing action carries.

**UI** (`billing-tab.tsx`) — plan `<Select>` auto-fills seats to the new ceiling; an upgrade/downgrade badge with feature gained/lost annotations appears once a different plan is picked; the seat blocker is surfaced inline (`data-testid="seats-below-usage"`) and disables the "Change plan" button before the confirm dialog can even open; the dialog's blast radius comes from a new `planChangeBlastRadius()` in `blast-radius.ts` (names the exact AI-agent-lock count and states the safe-fallthrough guarantee, or "none are currently configured, so nothing is locked" when there's nothing to lock).

**Creation-path bug closed**: `new-tenant-form.tsx`'s free-text plan `<Input>` (which let off-catalogue plans exist and get priced at $0 by `mrr.ts`) is now the same catalogue `<Select>`; `POST /api/platform/tenants` validates with `isValidPlanChange` server-side, same 400 shape as the billing route.

**Tests**: `plan-catalog.test.ts` +14 (rank monotonicity, `comparePlans` both directions incl. the premium→pro case, `describePlanChange` for every branch), `blast-radius.test.ts` +5 (a real bug caught by the new tests: the initial implementation rendered a negative price delta as `$-300/mo` instead of `-$300/mo`, fixed before merge). **No route-level test** — this plane has zero `*.test.ts` files under `api/platform/` by established convention (Playwright e2e covers it instead); putting the decision logic in `plan-catalog.ts` is what makes it unit-testable without inventing a new mocking pattern.

### 35.2 Caller-ID routing rules — independent build, explicitly NOT a Tel-Agent fork

**Context worth recording precisely, since it involved a real correction mid-session.** The user asked to inspect `github.com/Dpro-at/Tel-Agent` (an open-source AI phone assistant, AGPL-3.0) and "fork anything useful." Tel-Agent's own README states the AGPL network clause explicitly: *"If you run a modified version of Tel-Agent as a network service, you must publish your modifications."* The user then asked to both copy their code **and** give no source to anyone — those two things cannot coexist lawfully. This was flagged directly, and the user confirmed their actual goal: find the capability gap, build an equivalent, keep the product fully closed. There was also a genuine confusion mid-thread about "the current dograh fork" — **this repo has no Dograh fork**; `ai-voice-agent/` is forked from `ictinnovations/asterisk-ai-voice-agent` (MIT), specifically because Dograh needs ARI, which conflicts with this project's locked AMI-only rule (§34). Resolved by direct clarifying question before proceeding.

**What was actually built: zero code, text, or schema copied from Tel-Agent anywhere.** Only their *feature set* was studied (ideas aren't copyrightable, their implementation is). The one gap judged worth closing: Tel-Agent's caller-ID routing (pass/block/ai per contact identity, business-hours-aware) has no equivalent in this codebase — `DNC_CHECK()` is block-only and outbound-compliance-scoped, and `AI_INBOUND_EXTENSION()` decides AI-vs-queue purely by Dinstar port, never by who's calling. Independently designed from scratch against this codebase's own conventions (mirrors `DoNotCallEntry`'s shape, not Tel-Agent's SQLAlchemy `Rule` model), with one design simplification Tel-Agent doesn't have available: business hours are Asterisk's native `GotoIfTime()`, not a hand-rolled day/time parser (not implemented in this pass — see below).

**Schema** (`prisma/schema.prisma`, migration `20260914140000_add_caller_routing_rules`): new `CallerRoutingAction` enum (`PASS`/`BLOCK`/`AI`) and `CallerRoutingRule` model (`tenantId`, `pattern`, `action`, `note`), `@@unique([tenantId, pattern])`. Registered in both `src/lib/tenancy/scope-rules.ts`'s `TENANT_SCOPED_MODELS` and `scripts/lib/tenancy-tables.ts` (required for `tenantDb()` to scope it instead of throwing) — **deliberately NOT added to the RLS policy set**, same considered-not-oversight precedent `GeoLoginEvent`/`ExtensionUnlockRequest` already established (no reachable Postgres in this environment to write and verify a new RLS migration against).

**`src/lib/caller-routing.ts`** — pure `matchCallerRule(callerNumber, rules)`: longest-prefix-wins (an exact match is the degenerate case of a full-length "prefix"), case-insensitive, DB-free. Used by nothing at runtime yet except its own tests and (indirectly, as the spec the SQL is written to agree with) `func_odbc.conf`'s new ODBC function — the two are NOT sharing code, they're independently written to the same matching rule.

**Dialplan** — `func_odbc.conf` gained `[CALLER_ROUTING_ACTION]`: `SELECT "action"::text FROM "CallerRoutingRule" WHERE '${SQL_ESC(${ARG1})}' LIKE REPLACE("pattern",'*','%') ORDER BY LENGTH("pattern") DESC LIMIT 1`, empty-on-no-match (same fail-to-existing-behavior polarity as every ODBC function in this file). **Not tenant-scoped in the SQL**, for the identical reason `DNC_CHECK()` above it isn't: `[from-dinstar]` has no real per-call tenant identifier to pass in (`AI_TENANT_ID` is still hardcoded empty — §34's own standing gap). `extensions.conf`'s `[from-dinstar]` `s` extension now checks it right after the jitterbuffer setup, **before** the existing AI routing decision: `BLOCK` → `Goto(hangup,1)` (the context's existing shared hangup extension); `PASS` → jumps past the AI check straight to a newly-labelled `Queue(support_queue,t,,,300)` priority (`same => n(force-queue),...`), guaranteeing a human regardless of which Dinstar port or AI-agent config would otherwise apply; `AI` or no match falls through unchanged.

**API + UI** — `GET/POST /api/admin/caller-routing` and `DELETE /api/admin/caller-routing/[id]`, admin-only (`requireAdminSession`, not staff — this is a tenant-policy decision, not a day-to-day agent action, unlike DNC which staff can edit). Exact-number patterns are normalized through the same `normalizeToE164()` DNC already uses; prefix patterns (ending in `*`) are stored as typed. P2002→409 mapping follows the house convention (`admin/ai/agents/route.ts`'s dinstarPort handling). New admin page `/admin/caller-routing`, mirroring `/admin/dnc`'s add-form + list + confirm-remove shape (minus DNC's bulk-import machinery — this list is expected to hold a handful of policy rows, not a spreadsheet), linked from the sidebar nav under Configuration.

**Explicitly NOT built, and why**: no `take_message` tool (this session's own `request_callback`, §34.5/§34.6, already covers it — a second parallel primitive wasn't worth it); no channel-extension contract or new messaging channels (out of product scope — WhatsApp/SMS already via OpenWA); no LiveKit, no change to the AudioSocket/AMI transport (re-platforming would reopen the ARI-vs-AMI decision §34 already locked); no business-hours column on the DB model (Asterisk's native `GotoIfTime()` covers it, but wiring it into the `PASS` branch specifically was **not done this pass** — the dialplan change above doesn't yet wrap `PASS` in a business-hours check, so today a `PASS` rule applies at all hours; a real, stated gap, not silently glossed over).

**Tests**: `caller-routing.test.ts` (new, 8) — longest-prefix-wins, exact-beats-prefix, case-insensitivity, no-match, empty-rules. `api/admin/caller-routing/route.test.ts` (new, 9) + `[id]/route.test.ts` (new, 3) — auth, validation, E.164 normalization vs. prefix-stored-as-typed, P2002→409, delete-swallows-missing-id.

**Gates, all green (both 35.1 and 35.2 together):** `npx prisma generate` clean; `npm run typecheck` clean; `npm run test` — **1074/1074** (93 files, 39 new across both features); `npm run lint` clean; `npm run build` clean production build, `/admin/caller-routing` + both new API routes confirmed in the manifest. New migration (`20260914140000_add_caller_routing_rules`) hand-written and syntax-reviewed against the `AiCallSession`/`DoNotCallEntry` precedents — same standing "no reachable Postgres in this environment to diff-verify" caveat every migration in this repo carries. **Neither feature has been live-tested** — same standing caveat as everything else in this session; the caller-routing dialplan change in particular needs one real call per outcome (`BLOCK` refused, `PASS` reaches the queue regardless of AI config, no-rule unchanged) before being trusted in production.

**Committed and deployed** (both corrections to this section's own prior text, which was written mid-session before either happened): committed as `a758a5c` (plan upgrade/downgrade), `e944445` (caller routing), `5010c74` (docs) — all pushed to `origin/main`. **Confirmed live on the production VPS as of 2026-09-15**: `algo-web`'s image was built and the container restarted 2026-09-14T14:48:49Z (this had already happened before this handoff/LLM.md text was written — never previously recorded), all four new migrations (`20260914090000`..`20260914140000`) show `finished_at` in `_prisma_migrations`, `/admin/caller-routing` and its API routes are present in the deployed `.next/server` build, `/api/health`/`/login`/`/platform/login`/apex all return 200, zero errors in 17h of `algo-web` logs. **Still true and still open**: neither feature's actual call-path/business behavior has been live-tested (real BLOCK/PASS/AI calls; a real plan downgrade against a tenant with extensions in use) — only that the code is deployed and the app is healthy.

## 36. Conversation-workflow builder + real model configuration — sidecar prerequisite fixes, schema/API, Python interpreter (CASCADE + REALTIME/OpenAI), voice-picker fix, React Flow canvas UI (2026-09-15, follow-up to §34/§35)

Owner's ask, traced back to a real miscommunication: §34's plan (`~/.claude/plans/we-need-to-bring-pure-muffin.md`, line 11) explicitly dropped the upstream ictinnovations UI and rebuilt AI-agent config as flat form fields — one `systemPrompt`, no graph. The owner was picturing Dograh's visual node-graph builder the whole time. Full design at `~/.claude/plans/currently-we-have-forked-mutable-giraffe.md`. Explored via 2 parallel research agents (sidecar architecture, admin UI/schema) plus live WebFetch research into Dograh (BSD-2, confirmed no code/schema copied — same independent-build discipline as §35.2's Tel-Agent gap analysis) before planning.

**Decisions locked in with the owner:** build independently, Dograh as reference only; no re-platform onto Pipecat; node types Start Call/Agent/End Call/Transfer/Global/HTTP Tool + variable extraction; **both CASCADE and REALTIME**, except **Gemini Live is blocked for workflows** (its `setup` message is send-once-at-connection, no mid-session instruction swap — OpenAI Realtime's `session.update` is genuinely idempotent and does support it); single release, not phased rollout.

**Sidecar prerequisite fixes (`ai-voice-agent/`), all real bugs, not new scope:**
- **Tool results were never fed back to the LLM.** `ChatMessage` (`pipeline/base.py`) gained `tool_calls`/`tool_call_id`/`name`; `_handle_tool_call` now returns a result dict; `runner.py`'s new `_generate_turn_reply()` appends the assistant-with-tool_calls + `role="tool"` pair and re-enters `llm.generate()` up to `MAX_TOOL_ROUNDS=3`. `openai_compatible.py`/`gemini.py` gained wire-format serializers for both message shapes. Realtime gained a parallel fix: `RealtimeProvider.send_tool_result()`/`update_session()` added to the Protocol, implemented for real in `OpenAiRealtime` (stores `self._ws` during `run()`), and as a documented, logged no-op in `GeminiRealtime` (its class docstring now states why).
- **`ProviderLegConfig.extra` was always empty in production** — `config_client.LegConfig` had no `extra` field at all. Fixed; `_parse_leg` now reads `temperature`/`maxTokens`/`language`/`speed` into it generically.
- **Gemini Live silently dropped the system prompt** — `GeminiRealtime` had no `system_prompt` field or `systemInstruction` in `setup`. Fixed, `build_realtime()` now passes it through for both realtime providers.
- History cap (`_trim_history`, 40 non-system turns, never splits a tool_calls/tool pair), VAD/barge-in constants moved to instance fields (`voice_energy_threshold`, `end_of_utterance_silent_frames`) defaulting to the old hardcoded values.
- **Gates:** `ai-voice-agent` test suite **97/97** after this round (78 prior + 19 new: `test_tool_calling.py`, `test_pipeline_runner.py`, new `test_realtime_tool_result.py`, `test_config_client.py`).

**Schema (`prisma/schema.prisma`, migration `20260915120000_add_ai_workflows`, strictly additive).** `AiAgent.promptMode` (`"SIMPLE"` default | `"WORKFLOW"`) plus model-config columns (`llmTemperature`, `llmMaxTokens`, `ttsSpeed`, `sttLanguage`, `allowInterruption` default true, four VAD/barge-in overrides) — every existing row, including the live production tenant, is untouched. New `AiWorkflow` (draft blob + `publishedVersionId`), `AiWorkflowVersion` (immutable snapshots, `@@unique([workflowId, version])`), `AiWorkflowSecret` (HTTP-tool auth headers, AES-256-GCM via the existing `crypto.ts`). `AiCallSession` gained `gatheredContext`/`nodePath` for workflow-call debuggability. All three new models registered in both `scope-rules.ts` and `tenancy-tables.ts`; deliberately **not** added to RLS (same `CallerRoutingRule` precedent, §35.2 — no reachable Postgres to verify a new policy migration). Back-compat is structural, not just tested: `promptMode` defaults `SIMPLE`, a `WORKFLOW` agent with no *published* version also resolves to `SIMPLE` server-side (`agent-config/route.ts`), and `systemPrompt` is reinterpreted as the graph's global instructions rather than removed — switching modes loses nothing.

**Validation (`src/lib/ai/workflow-schema.ts`, new) — the single validator both the API and the canvas UI import; 28 tests.** Zod shape (`schemaVersion: literal(1)`, discriminated union of 6 node kinds, edges carrying the LLM-facing `condition` string) plus `validateWorkflowGraph()`: exactly one Start Call, no dangling/self edges, Global has no edges, End Call/Transfer are terminal, Agent nodes need a prompt and >=1 outgoing edge, duplicate edge conditions per node rejected (they become tool names), unreachable-from-start is a warning not an error, >=1 reachable terminal required, template refs (gathered_context/initial_context) checked against declared variables and a fixed allowlist. `parseWorkflowForPublish()` adds the one cross-cutting rule: `WORKFLOW` + `REALTIME` + Gemini => rejected with a clear message.

**Runtime interpreter (`ai-voice-agent/pipeline/workflow.py`, new — pure, no I/O, 18 tests) + `runner.py` wiring (12 tests in `test_workflow_runner.py`).** `WorkflowGraph.from_payload()`, `pathway_tool_map()`/`pathway_tools()` (one `ToolSpec` per outgoing edge, name = slug(condition)+edge-id-suffix, description = condition verbatim — that string IS the mechanism), `extraction_tool()` (one `record_info` tool covering every declared variable, not one per variable), `render()` (plain substitution, no eval), `system_prompt_for()` (agent prompt -> Global node -> node prompt, all rendered).

`PipelineRunner._run_workflow_cascade()`: conversation history is continuous across nodes, only the system message is rebuilt per round from the current node; per-node LLM/TTS provider instances built lazily and cached (`_leg_for_node`, keyed by resolved credential or falling back to the agent's own default leg); transitions happen only at tool-call granularity; `HTTP_TOOL`/`END_CALL`/`TRANSFER` nodes are automatic (no caller utterance collected) — `_drain_automatic_nodes()` runs the HTTP call chain and, on landing on End Call, speaks its `prompt` directly (never asks the model to improvise a farewell); on landing on Transfer, reuses `EscalationController` directly (stated simplification: a failed workflow transfer ends the workflow rather than reverting to free-form chat, since Transfer nodes have no outgoing edges to fall back to).

`PipelineRunner._run_workflow_realtime()` (new, added after initially being missed — see below): OpenAI Realtime only; one persistent session drives the whole call (no per-node provider swap, unlike cascade — a real, stated simplification), node transitions happen via `provider.update_session(new_instructions, new_tools)` inside the `on_tool_call` callback; landing on End Call sends a goodbye instruction with an empty tool set (the sidecar has no AMI access to force a hangup — same standing limitation as everywhere else in this file); Gemini reaching this method at all falls back to the plain single-prompt realtime loop, defense-in-depth behind the publish-time validator.

**Real bug caught and fixed before merge, not glossed over:** the first pass wired `start()` to route `is_workflow` only through `_run_workflow_cascade()` — a published `WORKFLOW` agent on `REALTIME` + OpenAI would have silently ignored its own graph and run the flat single-prompt loop, exactly the kind of "looks supported, isn't" gap this repo's own conventions warn against. Caught during self-review before declaring the session done, not by a test failure; `_run_workflow_realtime()` was added and `start()`'s dispatch fixed (`is_realtime and is_workflow` now checked first) as a direct result. A second real bug in the same new method: `node_path` wasn't appended for the node landed on before draining automatic nodes (both at call start and after a `goto_*`) — caught immediately by the new tests failing on first run, fixed by mirroring the append-before-drain order `_run_workflow_cascade` already used.

**Contract (`src/lib/ai/types.ts`, additive only) + `agent-config/route.ts`.** `AiAgentConfigResponse` gained `promptMode`, `workflow?: AiWorkflowRuntime`, `llm.temperature/maxTokens`, `tts.speed/baseUrl`, `stt.language`, `realtime.baseUrl`, `vad{}`. `agent-config/route.ts` resolves every node's `modelOverride` credential **once, deduped by credential id** across the whole graph (a per-request `Map` cache) before responding — the sidecar stays completely credential-ignorant, same as every top-level leg. `sttLanguageTag()` (new `src/lib/ai/languages.ts`, ~30 BCP-47 entries with per-provider tag mapping — Sarvam wants `hi-IN`, Deepgram wants `hi`) now derives a provider-correct STT language automatically from `AiAgent.language` when no explicit override is set, closing a second real gap (no mapping logic needed in the Python sidecar). 15 tests on the route (12 prior + back-compat regression guards: `SIMPLE` emits `workflow: null`, unpublished `WORKFLOW` also emits null, published agent emits resolved+deduped per-node creds).

**Workflow API (`src/app/api/admin/ai/agents/[id]/workflow/{route.ts,publish/route.ts}`, new, 21 tests).** `GET`/`PUT` on the draft (lenient — stores regardless of validity so an in-progress canvas edit is never lost, returns `issues[]`); `POST .../publish` strict-parses, validates every node's `modelOverride` credential and Transfer-target extension against the tenant (same `CREDENTIAL_ID_FIELDS` pattern as the agent editor), creates `AiWorkflowVersion` `max+1` in a `$transaction`, repoints `publishedVersionId`; also accepts `{revertToVersionId}` for one-click rollback with no new version row. **Fixed a real, independent security gap found while in this file:** `PATCH /api/admin/ai/agents/[id]` had no `planHasFeature` gate at all (only `POST` did) — a tenant whose plan lost `aiAgents` (or a downgrade-locked agent, `billing/route.ts`) could still edit an existing agent's config indefinitely. Fixed, tested.

**HTTP-tool node (`src/app/api/internal/ai/http-tool/route.ts`, new, 11 tests) — executed server-side, never in the sidecar.** The sidecar runs `network_mode: host`; letting it fetch a tenant-authored URL directly would be strictly worse than the SSRF surface already fixed in §34. New `ai-voice-agent/http_tool_client.py` sends only `{agentId, nodeId, gatheredContext}` (gathered_context threaded through specifically so template refs in the URL/headers/body can render — a gap caught before it shipped: the first draft only sent agentId/nodeId). The route re-reads the node from the **published** version only (never trusts anything the sidecar might send), resolves secrets from `AiWorkflowSecret` server-side, enforces the existing `assertPublicHttpUrl()` (§34's SSRF guard, reused unchanged), maps the JSON response into `gathered_context` via dot-path `responseMapping`.

**Voice-picker fix (blocker found during design, not new scope) — `src/lib/ai/providers/{elevenlabs,cartesia,openai}.ts`.** ElevenLabs and Cartesia were tagging voices `capabilities:["tts"]` — the SAME capability as real models — so the TTS **model** dropdown was already showing ~100 voice IDs mixed with 2-3 real models, a live bug independent of this feature. Added `"voice"` to `AiModelCapability`; both adapters now tag distinctly. Cartesia had no model list at all (only ever called `/voices`) — added a static 3-entry catalog (`sonic-2`/`sonic-english`/`sonic-multilingual`, matching the sidecar's own hardcoded default). OpenAI's `/v1/models` was classifying every non-realtime model `"llm"` including `tts-1`/`whisper-1` — fixed classification (`tts`/`stt`/dropped for embeddings) and added the 6 fixed OpenAI voices as a static catalog. `modelsFor()`/`voicesFor()` in the (now-shared, see below) leg-picker module filter on the correct capability — this alone fixes the dropdown pollution as a side effect. 13 tests across 3 provider files (2 new for Cartesia).

**Model-tuning UI + voice/speed picker (`src/app/admin/ai-agents/[id]/page.tsx`).** New "Model tuning" card (temperature, max tokens, allow-interruption toggle); TTS leg's free-text voice input replaced by `VoicePicker` (real `<Select>` when the provider has a voice list, free-text fallback otherwise) + a speed slider; language `<Select>` now sourced from `languages.ts`'s ~30-entry catalog instead of a hardcoded 4 (en/hi/hi-en/ar); a Simple<->Workflow switch + "Open workflow builder ->" link. `LegPicker`/`VoicePicker`/`Credential`/`modelsFor`/`voicesFor` extracted to `src/components/ai-agents/leg-pickers.tsx` (page.tsx files can't have arbitrary named exports under the App Router) so the new node property panel reuses them exactly, not a re-implementation.

**Builder UI (`@xyflow/react` 12.11.6, new dependency, MIT) — `src/components/ai-workflow/**`, `src/app/admin/ai-agents/[id]/workflow/page.tsx` (new dedicated route).** `WorkflowCanvas.tsx` (stateful host: domain-model refs, `useNodesState`/`useEdgesState`, 1.5s-debounced draft autosave, publish/revert), `nodes.tsx` (one shared `GenericNode` renderer parameterized by kind-specific icon/accent, rather than 6 near-identical files), `NodePropertiesPanel.tsx` (label/prompt/interruption/variables editor + kind-specific fields + collapsed model-override section reusing `LegPicker`/`VoicePicker`), `EdgeConditionDialog.tsx` (reuses the existing `Dialog` primitive), `ValidationPanel.tsx` (floating issues list, click-to-focus), `PublishBar.tsx` (publish disabled while blocking issues exist, version dropdown, revert). `workflow-canvas.css` overrides React Flow's own CSS custom properties to the Apple-black token system rather than fighting individual element rules — the documented React Flow theming mechanism.

**Gates, all green:** frontend `npx prisma generate` clean; `npm run typecheck` clean; `npm run test` — **1150/1150** (99 files, 76 new since §35's 1074); `npm run lint` clean (including the new UI files); `npm run build` — clean production build, `/admin/ai-agents/[id]/workflow` (79.6kB) + both new API routes confirmed in the manifest. Sidecar: `python -m pytest tests` — **127/127** (97 after prerequisite fixes + 30 new across `test_workflow_graph.py`/`test_workflow_runner.py`).

**NOT done, stated plainly, not glossed over:**
- **Nothing here has touched a live Asterisk instance or a real LLM.** Every protocol fix (OpenAI Realtime `function_call_output`, Gemini `toolResponse`, `update_session`) is written from public docs against modules that already carried "reconstructed from public docs, not verified live" before this session — the fixes are exactly as unverified as the code they fix.
- **Whether a real LLM reliably calls `goto_*`/`record_info` tools** is an empirical question no fake-LLM unit test can answer.
- **Turn latency**: the tool-feedback re-generate loop roughly doubles worst-case LLM legs per turn, and the system prompt is rebuilt every round. Needs a real call to measure, not assumed acceptable.
- **`allowInterruption=false` actually suppressing barge-in end-to-end** through `PacedPlayer.flush()` depends on real jitter/caller audio energy, not synthetic frames.
- **The migration** (`20260915120000_add_ai_workflows`) is hand-written and syntax-reviewed against the `20260914130000_add_ai_agent_handoff` precedent — same standing "no reachable Postgres in this environment" caveat every migration in this repo carries, never `prisma migrate diff`-verified.
- **`TRANSFER` nodes still hit the single-active-SIM 503** recorded in `src/lib/transfer-guard.ts` (§34.2) — a workflow with a Transfer path is no more testable today than the existing ad-hoc escalation tool.
- **Cartesia/Sarvam/Deepgram speed and language parameter names** in `extra` — this plumbing has literally never executed in production; only the OpenAI-compatible path (`temperature`/`max_tokens`) has any real-world precedent among comparable providers.
- **Realtime workflows lose per-node model overrides** by design (one persistent vendor session per call) — documented as a stated simplification, not silently dropped.
- **Nothing committed, nothing deployed, nothing pushed.** Left in the working tree deliberately, matching every other large feature pass in this file, pending the owner's review and go-ahead.

Before this reaches a real tenant: (1) live-test the ConfBridge/AudioSocket path this feature adds nothing new to but depends on (§34's own standing item); (2) place at least one real call through a published CASCADE workflow and one through a published OpenAI-realtime workflow, confirming `goto_*` calls actually fire and `update_session` actually moves the live session; (3) review the full diff against this section's file list; (4) commit, then decide on deploy per the same VPS method §35 used.
