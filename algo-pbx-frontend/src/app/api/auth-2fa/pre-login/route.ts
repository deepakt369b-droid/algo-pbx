...
export async function POST(request: NextRequest) {
  ...
  // Role rides along on every success response so the client can land the
  // user directly on their own workspace (/admin vs /agent) instead of
  // hardcoding /admin and relying on middleware to bounce agents off it
  // afterwards. Role is not sensitive at this stage — the caller just
  // proved the password for this exact account.
  const role = user.role;

  if (trusted) {
    const response = NextResponse.json({ needs2fa: false, role });
