...
export const SETTINGS_REGISTRY: SettingDef[] = [
  // --- Email (Resend) ---
  {
    ...
    help: "https://resend.com/api-keys — used to deliver agent invite/reset emails and, if GATEWAY_ALERT_EMAIL is set, Dinstar gateway critical-event alerts.",
    secret: true,
    envFallback: "RESEND_API_KEY",
    validator: z.string().min(1),
  },
  {
    key: "GATEWAY_ALERT_EMAIL",
    section: "email",
    label: "Gateway Alert Email",
    help: "Where critical Dinstar gateway syslog events (FORBID CALL, port unregistered, SIP trunk unreachable) are emailed. Requires RESEND_API_KEY — alerts stay in-app-only (the gateway alert banner on /admin/system) if this or the API key is unset.",
    secret: false,
    envFallback: "GATEWAY_ALERT_EMAIL",
  ...
  // --- SMS: Dinstar UC2000 ---
  {
    ...
    help: "The gateway's LAN/Tailscale IP — the voice trunk and SMS API both use it.",
    secret: false,
