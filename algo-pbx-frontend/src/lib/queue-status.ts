    ...
    queues.map(async (q): Promise<QueueSnapshot> => {
      try {
        ...
        const liveStatusByExtension = new Map(
          memberEvents.map((e) => {
            const ext = extensionFromInterface(e.Interface);
            // QueueMember carries a Paused flag ("yes"/"no") alongside the
            // device state — a paused member is deliberately out of rotation
            // (QueuePause) and the queue manager UI needs to see that to
            // offer Unpause.
            const paused = (e.Paused ?? "").toLowerCase() === "yes";
            return [ext, paused ? ("PAUSED" as const) : mapQueueMemberStatus(e.Status)] as const;
          }).filter((entry): entry is [string, AgentStatus | "PAUSED"] => entry[0] !== null)
        );
