...
// React Bits <Scanner /> (https://reactbits.dev — MIT), adapted to
// TypeScript for this project. Full-bleed animated scan-field background
// for the landing page, tuned to Algo PBX brand colors by the caller.
//
// Client-only WebGL: rendered via next/dynamic({ ssr: false }) from a
// CLIENT component page (src/app/page.tsx) — see that file's comment for
// why ssr:false inside a Server Component does NOT keep the module out of
// the server bundle in Next 14.
//
// The component itself already handles: ResizeObserver sizing,
// IntersectionObserver + document.visibilitychange render pausing, pointer
// focus, and full teardown (WEBGL_lose_context). Reduced-motion / no-WebGL
// fallback gating lives in the page-level wrapper, same as the previous
// ShaderGradient background did.
...
const ctxMap = new WeakMap<
  HTMLElement,
  { renderer: InstanceType<typeof Renderer>; program: InstanceType<typeof Program> }
>();
...
const Scanner = ({
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- uniform updates handled below; init once
  }, []);
