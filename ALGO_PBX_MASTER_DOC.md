...
graph TD
    ...
    subgraph Cloud_Server [Ubuntu Cloud VM — Docker Compose]
        TS_Cloud --> Core_PBX[Asterisk 20 PBX Engine\nnetwork_mode: host]
        Core_PBX <--> Coturn[Coturn STUN/TURN\nnetwork_mode: host]
        Core_PBX <--> PG[(PostgreSQL 16\nalgopbx_db + openwa DB)]
        Caddy[Caddy Reverse Proxy\nterminates 443 TLS] -- "HTTP :3000" --> Web_App[Algo PBX Web App\nNext.js 14 standalone]
        Web_App <--> PG
        Web_App <--> Core_PBX
        CDR[cdr-listener\nAMI → POST /api/cdr] --> Web_App
        OpenWA[OpenWA WhatsApp sidecar\n4 sessions = 4 SIMs] <--> PG
        Web_App <--> OpenWA
    end
    ...
    Agent1 & Agent2 -- "HTTPS 443 / WSS 8089" --> Caddy & Core_PBX
    Coturn -- "SRTP relay (UDP 20001-30000)" --> Agent1 & Agent2
    Core_PBX -- "RTP (UDP 10000-20000)" --> Agent1 & Agent2
    Core_PBX -- "SIP UDP 5060" --> GP
    Core_PBX == "Routed SIP via Tailscale" ==> DG

```

> **Live-service inventory note:** the compose stack actually runs **six**
> services (`postgres`, `coturn`, `asterisk`, `web`, `cdr-listener`,
> `openwa`) plus the `caddy` reverse proxy. The diagram above reflects that;
> the §6.1 sample below is the historical two-service sketch (see its banner).
...
> **⚠️ HISTORICAL REFERENCE ONLY.** The samples in this section are the
> original design sketches, kept for context. They are **superseded by the
> real files in this repo** and must not be deployed from here:
> - The compose sample hardcodes secrets and predates the Asterisk/Coturn
>   RTP port-range split (10000–20000 vs 20001–30000), the `openwa` and
>   `cdr-listener` services, healthchecks, resource limits, and the Caddy
>   reverse proxy. **The source of truth is `docker-compose.yml`.**
> - The `pjsip.conf`/`rtp.conf`/`extensions.conf` samples show hardcoded
>   endpoints and `REPLACE_ME_*` placeholders; extensions are now generated
>   into `pjsip_dynamic.conf` by `src/lib/pjsip-config.ts`, the Dinstar
>   trunk IP into `pjsip_dinstar.conf` by the `/admin/dinstar` wizard, and
>   `http.conf`/`asterisk.conf`/`manager.conf`/`queues.conf` etc. exist
>   beyond this sample set. **The source of truth is `pbx_configs/`.**
> - The `SIPContext.tsx` sample below mixes raw `UserAgent`/`Web.SimpleUser`
>   APIs and build-time env credentials; the real implementation
>   (`src/contexts/sip-context.tsx`) runs on `Web.SessionManager`, fetches
>   per-agent SIP credentials from `/api/me/sip-credentials`, and TURN
>   config from `/api/config/public`.
> - Coturn auth is via REST ephemeral credentials minted per-agent
>   (`api/me/turn-credentials`), not the static `turnuser`/`turnpwd`
>   shown in the rtp.conf sample.
