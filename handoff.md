# Handoff — SaaS owner console at `/platform` BUILT (2026-09-06): all six sections, the billing ladder, provisioning, platform users, audit center, platform settings, plus the per-tenant recording delivery pipeline. Eight commits, all local and **unpushed**. Every commit gated on typecheck + lint + test + build (784 unit tests, up from 490). **Two migrations are written but NOT applied to production** — you asked for per-migration approval with before/after row counts, so they are waiting on you. **The Playwright acceptance suite (45 tests, 5 specs) has never been executed**: this environment has no database and no Docker. It is written and typechecks; running it needs a live stack and seeded local/staging credentials. One acceptance criterion is deliberately not automatable — "a real call completes while a tenant is suspended" — and is now a documented BLOCKER in `GO_LIVE_CHECKLIST.md` Gate 1b.

## ▶ "claude continue" — the remaining work, in order

Updated 2026-09-06. Remaining, in order:

1. **Apply the two migrations to production, one at a time, with your go-ahead each time.** `20260906100000_add_platform_console` and `20260906110000_add_recording_delivery`. Both are strictly additive — every new column is nullable with no default and no backfill, so no table is rewritten and both are reversible by dropping the columns/tables. Capture `SELECT count(*)` on `Tenant`, `PlatformUser` and `Recording` before and after each, per your instruction. Nothing in the console works against production until these land.
2. **Run the platform Playwright suite against a local or staging stack.** Needs `E2E_PLATFORM_EMAIL` / `_PASSWORD` / `_TOTP_SECRET` for a seeded account. **Do not create a platform test account on production** — `e2e/auth.setup.ts` refuses to run against a production host, and `e2e/README.md` explains why (a platform account can suspend, offboard, cut a dialplan and read every tenant's recordings).
...
4. **Create the one-time wildcard DNS record** `*.algopbx.com` (grey-cloud) before any tenant workspace can resolve. Then, and only then, enable the wildcard Caddy block from Platform Settings — a failed DNS-01 challenge for it is fatal to Caddy's ENTIRE config and would crash-loop the reverse proxy, taking the working production site with it. That is why it needs two separate confirmations.
