...
Four user-reported gaps closed: (1) `/platform/tenants` rows were inert — now clickable, opening a modal that reuses the EXISTING tenant-detail tabs (billing/users/geo, including the geo tab's already-real country dropdown — the real gap there was just no Extensions tab, now added) via a new `GET .../detail` JSON endpoint; (2) `/platform/provisioning/[id]` jumped straight into the editable wizard — now shows a read-only overview (progress/prereqs/blockers) first, with an explicit "Open edit wizard" action; (3) new per-user VPN profiles at `/admin/users/[id]` (tenant admin only, `requireAdminSession()`-gated, real X25519 WireGuard keypairs, one-time config disclosure, Rotate/Revoke); (4) CRM completion — deals now have a company picker + stage/currency/owner/expected-close, the task board got a "New task" dialog (contact required) + a POST route it never had, and threaded notes (activating the long-dormant `DealNote` model, new `CompanyNote`) now write into the unified `Activity` timeline on deals/companies/contacts. The CRM-analytics-dashboard ask turned out to already exist (`/admin/reports`'s "CRM Insights" tab, fully built) — reused directly on `/admin` itself rather than duplicated.

**Gates before deploy, all green**: `npx prisma generate` clean, `npm run typecheck` clean, `npm run test` 910/910 (43 new), `npm run lint` clean, `npm run build` clean production build.

**Deployed for real.** Committed (`668fcdc`) and pushed to `origin/main`. VPS `/opt/algo-pbx`: backed up pre-deploy state to `/root/pre-crm-vpn-deploy-20260908/` (commit hash + `git status`), `git fetch` + `git reset --hard origin/main` (`e67c1fa` → `668fcdc`, clean fast-forward, no divergence) — the one known untracked migration dir (`20260904140000_add_tenancy_constrain`) and `website/out` both confirmed to survive the reset, same as every prior deploy of this kind. `docker compose build --no-cache web` (clean), `docker compose up -d web` — the container's own entrypoint ran `prisma migrate deploy` and applied the one new migration (`20260908120000_add_user_vpn_and_crm_notes`) cleanly ("32 migrations found ... All migrations have been successfully applied"), app ready in 98ms. **Verified**: all 13 containers up, `algo-web` healthy; `/api/health` 200, `/login` 200, `/platform/login` 200, `/platform` + `/platform/tenants` 307→login (guard live); apex and `pbx.saharatechs.com` both 200 over HTTPS (`--resolve`, since public DNS routing wasn't re-checked); zero errors in `algo-web`'s startup logs; `UserVpnProfile`/`CompanyNote` tables and `ContactTask.description` column confirmed present via direct `psql` query, not just inferred from the migration log.

**Not run**: `prisma migrate diff` against a shadow DB before deploy — no reachable Postgres in the build environment, so the migration's shape was syntax-reviewed (modeled on the existing geo-lock migration) rather than diff-verified; it did apply cleanly against the real production DB, which is the stronger proof. **Nothing here has carried a live call or a real click-through since the deploy** — this is a UI/API-only feature set with no telephony-path changes, but the new panels (tenant modal, VPN generation, deal/task/note dialogs) have not yet been exercised by a human in production.

---

# Handoff — Owner-console UMS, multi-transport connectivity failover, and per-extension geo-lock are **DEPLOYED AND LIVE on production** (2026-09-07, now three deploys in one day). VPS `main` is at `e67c1fa`. Deploy 3 (this one) was a small follow-up gated the same way as the others (tsc/lint/867 tests/build, then VPS fast-forward + `docker compose build --no-cache web` + `up -d web`): **new tenants now default `geoLockMode` to `"monitor"`** (owner policy — every tenant created from now on starts recording geo evidence automatically, existing tenant untouched); the Provisioning tab's "Not started" tenants were a dead, unclickable list item and are now linked into the same wizard "In progress" tenants use (confirmed safe — the pipeline's first two steps are no-ops on an already-existing tenant); the Identity tab's edit button was genuinely easy to miss (now a bordered button with an icon, plus the tenant-name row itself is click-to-edit); and **a real bug fixed**: `LEGACY_TENANT_ONE_SLUG` was `"saharatechs"` (the domain) instead of `"sahara"` (the tenant's actual slug), so the Identity tab's "Workspace URL" was silently showing `sahara.algopbx.com` — a hostname with zero DNS/Caddy/cert configured, doesn't resolve — instead of the real working `pbx.saharatechs.com`. Verified post-deploy: all 13 containers healthy, `/api/health`/`/login`/`/platform/login` 200, `/platform`+`/platform/provisioning` 307→login, apex 200, zero container errors.

Deploy 2 (same day, earlier): owner-console UMS, multi-transport connectivity failover, and per-extension geo-lock went live. VPS moved `4e8fc91` → `96a4bb1` (2 commits: the feature, then a same-day geoip-refresh permission fix), migrations `20260908090000_add_gateway_failover` + `20260908100000_add_extension_geo_lock` applied via the established throwaway-container method. **Real bug caught live, not just in code review**: `geoip-refresh`'s first boot failed both file fetches (curl exit 23 — `curlimages/curl`'s default non-root user can't write a freshly-created root-owned named volume); fixed with `user: "0:0"` on that one throwaway-fetch container, committed, pushed, redeployed — confirmed both `GeoLite2-{Country,ASN}.mmdb` (8.4MB/12MB) now land correctly and are readable by `web`. **The whole geo-lock and auto-failover feature ships dormant by design**: `Tenant.failoverEnabled` defaults `false` for every tenant — nothing auto-repoints the trunk until an owner explicitly turns it on per tenant. (`geoLockMode` now defaults to `"monitor"`, not `null`, as of deploy 3 above — the very first per-tenant sentence of the plan's rollout is now baked into tenant creation itself, not a manual step.) Built via a coordinated multi-agent task graph, verified by three independent fresh-context reviewers before merge (one CRITICAL finding — the failover supervisor didn't actually prevent automatic fail-back — fixed pre-merge, not shipped and patched later). **Nothing here has carried a live call since any of these deploys** — Gate 1b is still open, see item 0 below, now joined by a second untested promise: that `failoverEnabled` really does sit inert until switched on.

---

## Prior handoff (2026-09-07, first deploy of the day) — preserved below

SaaS owner console at `/platform` **DEPLOYED AND LIVE on production**. The VPS moved `a37f2e7` → `4e8fc91` (22 commits) with owner go-ahead, rebuilt `--no-cache` one service at a time, and verified: `/api/health` `{"ok":true}`, `/login` 200, `/platform/login` 200, `/platform` 307 → login (guard live), the public apex site still 200 (`/` and `/terms`), `algo-web` healthy with zero errors, all 13 containers up. Both migrations were already applied (2026-09-06), so this was the "contract" half of expand/contract and the container's start-up `prisma migrate deploy` correctly found nothing pending. Everything the reset discarded is preserved under `/root/pre-console-deploy-20260907/` on the VPS. **Nothing here has carried a live call since the deploy** — Gate 1b (a real call completing while a tenant is suspended) is still the untested, load-bearing promise of the billing design, and is now item 0 below. Prior state: console built over ten commits, all gated on typecheck + lint + test + build (784 unit tests); Playwright acceptance suite run 2026-09-05 (28 passed, 0 failed, 5 skipped), which found one real product bug and three test defects, all fixed.

## ▶ "claude continue" — the remaining work, in order

Updated 2026-09-08 (owner tenant-management modal / provisioning overview / per-user VPN / CRM completion session).
Remaining, in order:

-2. ~~Review and commit today's (2026-09-08) working-tree changes, then decide whether to deploy.~~ **DONE 2026-09-08 — committed (`668fcdc`), pushed, and DEPLOYED.** See the top handoff entry. **Still worth doing when convenient**: a real click-through of the new panels (tenant management modal, per-user VPN generation, deal company-picker, task creation, notes on deal/company/contact) — none of it has been exercised by a human against production yet, only smoke-checked (HTTP status codes + a direct DB column check).

-1. **Watch `GeoLoginEvent` in monitor mode before enabling `enforce`, and
   watch a tenant's transport probes agree with reality before enabling
   `failoverEnabled`.** The real Sahara tenant still has `geoLockMode`
   unset (`null`, off) — the new `"monitor"` default only applies to
   tenants created from here on, it was NOT retroactively applied to the
   one real tenant. If you want Sahara watched too, that's a separate,
   explicit owner action from its Geo tab. `failoverEnabled` stays `false`
   for every tenant, new or old, until switched on manually. Per the
   plan's H2/H3 gates, turning either on is the next real decision point,
   not an automatic follow-up. Do this AFTER Gate 1b below, not before —
   don't add a second untested automatic behavior on top of an
   already-untested one.
0. ~~DECIDE: deploy the owner console to the VPS, or keep holding.~~ **DONE
   2026-09-07 — DEPLOYED.** Owner chose deploy. Method and the three things
   the earlier pre-flight had NOT caught, each worth knowing before the next
   deploy of this kind:
   - **`website/out` is untracked AND absent from `origin/main`, but it IS the
     live public site** — `docker-compose.yml` bind-mounts it to
     `/srv/website` and the Caddyfile `root`s the apex at it. The obvious way
     to clear the checkout conflict (delete the untracked `website/` tree)
     would have taken the working public website down. It was moved aside and
     restored after the reset; apex verified 200 before and after.
   - **`20260904140000_add_tenancy_constrain` is applied in the production DB
     but does not exist in `origin/main`.** It was deliberately kept as an
     untracked directory so the migrations folder stays a superset of the DB.
     Start-up `prisma migrate deploy` then reported "29 migrations found / No
     pending migrations to apply". Cleaning it would have failed the container
     on an applied-but-missing migration. **Do not `git clean` that path.**
   - Everything discarded lives under **`/root/pre-console-deploy-20260907/`**
     (full `git diff`, `git status`, an untracked tarball, `website-out/`, and
     the displaced `website/`, `platform/setup`, `api/platform/setup` and
     `20260905090000` migration trees). Nothing was `rm`'d; the reset is
     reversible.
   - The build itself: `docker compose build --no-cache web` then
     `docker compose up -d web` — one service (2 vCPU), `--no-cache` because
     this pull adds many new files and cached builds have silently omitted
     new files on this box before.

0b. **THE NEXT REAL THING: Gate 1b, and it now matters more than it did.**
   Suspend a test tenant, then place a real inbound and a real outbound call
   and confirm both still connect. Until 2026-09-07 the billing ladder was
   code that had never run on production; it is now live in front of a real
   tenant. The design promise is that suspension gates UI login and NEVER
   telephony — asserted structurally in `enforcement.test.ts`, but never once
   demonstrated on real hardware. See item 3 (same check, unchanged) and
   `GO_LIVE_CHECKLIST.md` Gate 1b.
1. ~~Apply the two migrations to production.~~ **DONE 2026-09-06.** Both applied with individual go-ahead and before/after evidence: `20260906100000_add_platform_console` (13:22:18 UTC — 10 nullable `Tenant` columns, 4 nullable `PlatformUser` columns, 2 indexes) and `20260906110000_add_recording_delivery` (13:24:03 UTC — 2 empty tables, 2 enums, 4 FKs, 6 indexes). Row counts identical before and after both; every new column NULL on every existing row; `Recording` still has its original 8 columns. App verified healthy afterwards (`/login`, `/platform/login`, `/api/health` all HTTP 200, no container errors).
   ...
   - **The VPS working tree is now at `4e8fc91` (deployed 2026-09-07).** This note is superseded — it used to say the tree was deliberately held at `a37f2e7` so a rebuild would not silently deploy the console. The console has since been deployed; see item 0.
