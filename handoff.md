# Handoff — Owner-console UMS, multi-transport connectivity failover, and per-extension geo-lock are **DEPLOYED AND LIVE on production** (2026-09-07, second deploy of the day). VPS `main` moved `4e8fc91` → `96a4bb1` (2 commits: the feature, then a same-day geoip-refresh permission fix), migrations `20260908090000_add_gateway_failover` + `20260908100000_add_extension_geo_lock` applied via the established throwaway-container method, `docker compose build --no-cache web` (one service), `docker compose up -d web` then `up -d geoip-refresh` (new service). Verified: `/api/health` `{"ok":true}`, `/login` 200, `/platform/login` 200, `/platform` 307 → login, `/platform/geo-locks` 307 → login (new route compiled and guarded), apex 200, `algo-web` healthy with zero errors, all 13 containers up (12 prior + new `algo-geoip-refresh`). **Real bug caught live, not just in code review**: `geoip-refresh`'s first boot failed both file fetches (curl exit 23 — `curlimages/curl`'s default non-root user can't write a freshly-created root-owned named volume); fixed with `user: "0:0"` on that one throwaway-fetch container, committed, pushed, redeployed — confirmed both `GeoLite2-{Country,ASN}.mmdb` (8.4MB/12MB) now land correctly and are readable by `web`. **The whole geo-lock and auto-failover feature ships dormant by design**: `Tenant.geoLockMode` defaults `null` (off) and `Tenant.failoverEnabled` defaults `false` for every tenant — nothing auto-locks an agent out or auto-repoints the trunk until an owner explicitly turns it on per tenant, monitor mode first per the plan (`C:\Users\DK\.claude\plans\see-the-image-the-vast-lantern.md` §4). Built via a coordinated multi-agent task graph, verified by three independent fresh-context reviewers before merge (one CRITICAL finding — the failover supervisor didn't actually prevent automatic fail-back — fixed pre-merge, not shipped and patched later). **Nothing here has carried a live call since either deploy** — Gate 1b is still open, see item 0 below, now joined by a second untested promise: that `failoverEnabled` and `geoLockMode` really do sit inert until switched on.
...
-1. **Watch `GeoLoginEvent` in monitor mode before enabling `enforce`, and
   ...
   `failoverEnabled`.** Both features are live in code but dormant by
   config (`geoLockMode` null, `failoverEnabled` false for every tenant) —
   per the plan's H2/H3 gates. Turning either on for the one real tenant is
   the next real decision point, not an automatic follow-up. Do this
   AFTER Gate 1b below, not before — don't add a second untested
   automatic behavior on top of an already-untested one.
